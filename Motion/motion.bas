'******** 2О©╫О©╫ О©╫О©╫О©╫О©╫н╨О©╫ О©╫й╠О©╫ О©╫О©╫О©╫О©╫ О©╫О©╫О©╫н╠в╥О©╫ ********

'-*- coding: utf-8 -*-'

DIM I AS BYTE
DIM J AS BYTE
DIM MODE AS BYTE
DIM A AS BYTE
DIM A_old AS BYTE
DIM B AS BYTE
DIM C AS BYTE
DIM О©╫О©╫О©╫О©╫с╣О©╫ AS BYTE
DIM О©╫б©О©╫с╣О©╫ AS BYTE
DIM О©╫б©О©╫с╣О©╫2 AS BYTE
DIM О©╫О©╫О©╫О©╫О©╫О©╫О©╫ AS BYTE
DIM О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ AS BYTE
DIM О©╫О©╫О©╫О©╫ц╪е╘ AS BYTE
DIM О©╫О©╫О©╫О©╫ONOFF AS BYTE
DIM О©╫О©╫О©╫л╥О©╫ONOFF AS BYTE
DIM О©╫О©╫О©╫О©╫у╣О©╫ AS INTEGER
DIM О©╫О©╫О©╫О©╫О©╫б©О©╫ AS INTEGER

DIM О©╫Н╪╠О©╫О©╫О©╫О©╫ AS BYTE

DIM О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ AS BYTE
DIM О©╫О©╫О©╫О©╫х╝О©╫О©╫х╫О©╫О©╫ AS BYTE
DIM О©╫О©╫О©╫О©╫х╫О©╫О©╫ AS BYTE
DIM О©╫О©╫О©╫О©╫COUNT AS BYTE

DIM О©╫О©╫О©╫э╪О©╫О©╫е╦О©╫О©╫О©╫  AS BYTE

DIM S11  AS BYTE
DIM S16  AS BYTE
'**************************************
DIM NO_0 AS BYTE
DIM NO_1 AS BYTE
DIM NO_2 AS BYTE
DIM NO_3 AS BYTE
DIM NO_4 AS BYTE

DIM NUM AS BYTE

DIM BUTTON_NO AS INTEGER
DIM SOUND_BUSY AS BYTE
DIM TEMP_INTEGER AS INTEGER

'**** О©╫О©╫О©╫Б╪╬О©╫О©╫О©╫О©╫ф╝ О©╫О©╫О©╫О©╫ ****
CONST О©╫у╣з╠О©╫О©╫О©╫ADО©╫О©╫ф╝ = 0
CONST О©╫б©О©╫О©╫О©╫О©╫ADО©╫О©╫ф╝ = 1
CONST О©╫О©╫О©╫О©╫х╝О©╫н╫ц╟О©╫ = 20  'ms

CONST О©╫О©╫О©╫э╪О©╫ADО©╫О©╫ф╝  = 4


CONST min = 61	'О©╫з╥нЁя╬О©╫О©╫О©╫О©╫О©╫О©╫О©╫
CONST max = 107	'О©╫О©╫О©╫О©╫О©╫нЁя╬О©╫О©╫О©╫О©╫О©╫О©╫О©╫
CONST COUNT_MAX = 3


CONST О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫ = 10
'************************************************



PTP SETON 				'О©╫О©╫О©╫О©╫О©╫в╥Л╨╟ О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ О©╫О©╫О©╫О©╫
PTP ALLON				'О©╫О©╫ц╪О©╫О©╫О©╫О©╫ О©╫О©╫О©╫О©╫О©╫О©╫ О©╫О©╫О©╫О©╫ О©╫О©╫О©╫О©╫

DIR G6A,1,0,0,1,0,0		'О©╫О©╫О©╫О©╫0~5О©╫О©╫
DIR G6D,0,1,1,0,1,1		'О©╫О©╫О©╫О©╫18~23О©╫О©╫
DIR G6B,1,1,1,1,1,1		'О©╫О©╫О©╫О©╫6~11О©╫О©╫
DIR G6C,0,0,0,0,1,0		'О©╫О©╫О©╫О©╫12~17О©╫О©╫

'************************************************

OUT 52,0	'О©╫с╦О©╫ LED О©╫я╠О©╫
'***** О©╫й╠Б╪╠О©╫О©╫ '************************************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
О©╫О©╫О©╫О©╫ц╪е╘ = 0
О©╫О©╫О©╫О©╫х╝О©╫О©╫х╫О©╫О©╫ = 0
О©╫О©╫О©╫О©╫х╫О©╫О©╫ = 1
О©╫О©╫О©╫О©╫ONOFF = 0

'****О©╫й╠О©╫О©╫О©╫д║ О©╫г╣О©╫О©╫*****************************


TEMPO 230
'MUSIC "cdefg"



SPEED 5
GOSUB MOTOR_ON

S11 = MOTORIN(11)
S16 = MOTORIN(16)

SERVO 11, 100
SERVO 16, S16

SERVO 16, 100


GOSUB О©╫О©╫О©╫О©╫О©╫й╠О©╫О©╫з╪О©╫
GOSUB О©╫Б╨╩О©╫з╪О©╫


GOSUB О©╫О©╫О©╫л╥О©╫INIT
GOSUB О©╫О©╫О©╫л╥О©╫MID
GOSUB О©╫О©╫О©╫л╥О©╫ON



PRINT "VOLUME 200 !"
'PRINT "SOUND 12 !" 'О©╫хЁО©╫О©╫о╪О©╫О©╫О©╫

GOSUB All_motor_mode3





GOTO MAIN	'О©╫ц╦О©╫О©╫О©╫ О©╫О©╫О©╫О©╫ О©╫О©╫ф╬О©╫О©╫О©╫О©╫ О©╫О©╫О©╫О©╫

'************************************************

'*********************************************
' Infrared_Distance = 60 ' About 20cm
' Infrared_Distance = 50 ' About 25cm
' Infrared_Distance = 30 ' About 45cm
' Infrared_Distance = 20 ' About 65cm
' Infrared_Distance = 10 ' About 95cm
'*********************************************
'************************************************
О©╫О©╫О©╫О©╫О©╫О©╫:
    TEMPO 220
    'MUSIC "O23EAB7EA>3#C"
    RETURN
    '************************************************
О©╫О©╫О©╫О©╫О©╫О©╫:
    TEMPO 220
    'MUSIC "O38GD<BGD<BG"
    RETURN
    '************************************************
О©╫О©╫О©╫О©╫О©╫О©╫:
    TEMPO 250
    MUSIC "FFF"
    RETURN
    '************************************************
    '************************************************
MOTOR_ON: 'О©╫О©╫О©╫О©╫ф╝О©╫О©╫О©╫О©╫О©╫О©╫О©╫м╩О©╫К╪ЁО©╫О©╫

    GOSUB MOTOR_GET

    MOTOR G6B
    DELAY 50
    MOTOR G6C
    DELAY 50
    MOTOR G6A
    DELAY 50
    MOTOR G6D

    О©╫О©╫О©╫О©╫ONOFF = 0
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫			
    RETURN

    '************************************************
    'О©╫О©╫О©╫О©╫ф╝О©╫О©╫О©╫О©╫О©╫О©╫О©╫м╩О©╫К╪ЁО©╫О©╫
MOTOR_OFF:

    MOTOROFF G6B
    MOTOROFF G6C
    MOTOROFF G6A
    MOTOROFF G6D
    О©╫О©╫О©╫О©╫ONOFF = 1	
    GOSUB MOTOR_GET	
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫	
    RETURN
    '************************************************
    'О©╫О©╫д║О©╫О©╫О©╫г╣О©╫О©╫
MOTOR_GET:
    GETMOTORSET G6A,1,1,1,1,1,0
    GETMOTORSET G6B,1,1,1,0,0,1
    GETMOTORSET G6C,1,1,1,0,1,0
    GETMOTORSET G6D,1,1,1,1,1,0
    RETURN

    '************************************************
    'О©╫О©╫д║О©╫О©╫О©╫г╣О©╫О©╫
MOTOR_SET:
    GETMOTORSET G6A,1,1,1,1,1,0
    GETMOTORSET G6B,1,1,1,0,0,1
    GETMOTORSET G6C,1,1,1,0,1,0
    GETMOTORSET G6D,1,1,1,1,1,0
    RETURN

    '************************************************
All_motor_Reset:

    MOTORMODE G6A,1,1,1,1,1,1
    MOTORMODE G6D,1,1,1,1,1,1
    MOTORMODE G6B,1,1,1,,,1
    MOTORMODE G6C,1,1,1,,1

    RETURN
    '************************************************
All_motor_mode2:

    MOTORMODE G6A,2,2,2,2,2
    MOTORMODE G6D,2,2,2,2,2
    MOTORMODE G6B,2,2,2,,,2
    MOTORMODE G6C,2,2,2,,2

    RETURN
    '************************************************
All_motor_mode3:

    MOTORMODE G6A,3,3,3,3,3
    MOTORMODE G6D,3,3,3,3,3
    MOTORMODE G6B,3,3,3,,,3
    MOTORMODE G6C,3,3,3,,3

    RETURN
    '************************************************
Leg_motor_mode1:
    MOTORMODE G6A,1,1,1,1,1
    MOTORMODE G6D,1,1,1,1,1
    RETURN
    '************************************************
Leg_motor_mode2:
    MOTORMODE G6A,2,2,2,2,2
    MOTORMODE G6D,2,2,2,2,2
    RETURN

    '************************************************
Leg_motor_mode3:
    MOTORMODE G6A,3,3,3,3,3
    MOTORMODE G6D,3,3,3,3,3
    RETURN
    '************************************************
Leg_motor_mode4:
    MOTORMODE G6A,3,2,2,1,3
    MOTORMODE G6D,3,2,2,1,3
    RETURN
    '************************************************
Leg_motor_mode5:
    MOTORMODE G6A,3,2,2,1,2
    MOTORMODE G6D,3,2,2,1,2
    RETURN
    '************************************************
Arm_motor_mode1:
    MOTORMODE G6B,1,1,1,,,1
    MOTORMODE G6C,1,1,1,,1
    RETURN
    '************************************************
Arm_motor_mode2:
    MOTORMODE G6B,2,2,2,,,2
    MOTORMODE G6C,2,2,2,,2
    RETURN

    '************************************************
Arm_motor_mode3:
    MOTORMODE G6B,3,3,3,,,3
    MOTORMODE G6C,3,3,3,,3
    RETURN
    '************************************************

О©╫О©╫О©╫О©╫О©╫й╠О©╫О©╫з╪О©╫:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  90,
    MOVE G6C,100,  35,  90

    WAIT
    mode = 0
    RETURN
    '************************************************
О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫:
    MOVE G6A,98,  76, 145,  93, 101, 100
    MOVE G6D,98,  76, 145,  93, 101, 100
    MOVE G6B,100,  35,  90,
    MOVE G6C,100,  35,  90
    WAIT
    mode = 0

    RETURN
    '******************************************	


    '************************************************
<<<<<<< HEAD
О©╫Б╨╩О©╫з╪О©╫:
	
=======
╠Б╨╩юз╪╪:

>>>>>>> 5f3e757 (Chore: Л┐╓Л┌╜Л┐╓Л┌╜ К╙╗Л┘≤ Л┬≤Л═∙)
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  30,  80,
    MOVE G6C,100,  30,  80,
    WAIT
    mode = 0

    RETURN
    '******************************************	
О©╫Б╨╩О©╫з╪О©╫2:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  30,  80,
    MOVE G6C,100,  30,  80
    WAIT

    mode = 0
    RETURN
    '******************************************	
О©╫О©╫О©╫О©╫О©╫з╪О©╫:
    MOVE G6A,100, 56, 182, 76, 100, 100
    MOVE G6D,100, 56, 182, 76, 100, 100
    MOVE G6B,100,  30,  80,
    MOVE G6C,100,  30,  80
    WAIT
    mode = 2
    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫з╪О©╫:
    GOSUB О©╫О©╫О©╫л╥О©╫OFF
    MOVE G6A,100, 145,  28, 145, 100, 100
    MOVE G6D,100, 145,  28, 145, 100, 100
    MOVE G6B,100,  30,  80,
    MOVE G6C,100,  30,  80
    WAIT
    mode = 1

    RETURN
    '******************************************
    '***********************************************
    '***********************************************
    '**** О©╫О©╫О©╫л╥н╟О©╫О©╫О©╫ О©╫О©╫О©╫О©╫ ****
О©╫О©╫О©╫л╥О©╫INIT:

    GYRODIR G6A, 0, 0, 1, 0,0
    GYRODIR G6D, 1, 0, 1, 0,0

    GYROSENSE G6A,200,150,30,150,0
    GYROSENSE G6D,200,150,30,150,0

    RETURN
    '***********************************************
    '**** О©╫О©╫О©╫л╥н╟О©╫О©╫О©╫ О©╫О©╫О©╫О©╫ ****
О©╫О©╫О©╫л╥О©╫MAX:

    GYROSENSE G6A,250,180,30,180,0
    GYROSENSE G6D,250,180,30,180,0

    RETURN
    '***********************************************
О©╫О©╫О©╫л╥О©╫MID:

    GYROSENSE G6A,200,150,30,150,0
    GYROSENSE G6D,200,150,30,150,0

    RETURN
    '***********************************************
О©╫О©╫О©╫л╥О©╫MIN:

    GYROSENSE G6A,200,100,30,100,0
    GYROSENSE G6D,200,100,30,100,0
    RETURN
    '******************ZERO G6A,100, 101, 102, 106, 100, 100
    ZERO G6B,102, 101, 100, 100, 100, 100
    ZERO G6C, 97, 100,  96, 100, 100, 100
    ZERO G6D,100, 103, 103, 104, 103, 100
    '*****************************
О©╫О©╫О©╫л╥О©╫ON:

    GYROSET G6A, 4, 3, 3, 3, 0
    GYROSET G6D, 4, 3, 3, 3, 0

    О©╫О©╫О©╫л╥О©╫ONOFF = 1

    RETURN
    '***********************************************
О©╫О©╫О©╫л╥О©╫OFF:

    GYROSET G6A, 0, 0, 0, 0, 0
    GYROSET G6D, 0, 0, 0, 0, 0


    О©╫О©╫О©╫л╥О©╫ONOFF = 0
    RETURN

    '************************************************

    '******************************************
    '**********************************************
    '**********************************************
RX_EXIT:

    ERX 4800, A, MAIN

    GOTO RX_EXIT
    '**********************************************
GOSUB_RX_EXIT:

    ERX 4800, A, GOSUB_RX_EXIT2

    GOTO GOSUB_RX_EXIT

GOSUB_RX_EXIT2:
    RETURN
    '**********************************************
    '**********************************************


О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    SPEED 7
    HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 147,  93, 101
        MOVE G6D,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 147,  93, 101
        MOVE G6A,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF


    '**********************

О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_1: 'О©╫ч╧О©╫
    'HIGHSPEED SETON
    MOVE G6A,95,  90, 125, 100, 104
    MOVE G6D,106,  77, 147,  93,  102
    MOVE G6B, 85
    MOVE G6C,115
    WAIT


О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_2:

    MOVE G6A,103,   73, 140, 103,  100
    MOVE G6D, 95,  85, 147,  85, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

        GOTO RX_EXIT
    ENDIF

    ERX 4800,A, О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_2_stop:
        MOVE G6D,95,  90, 125, 95, 104
        MOVE G6A,104,  76, 145,  91,  102
        MOVE G6C, 100
        MOVE G6B,100
        WAIT

        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        'HIGHSPEED SETOFF
        GOTO RX_EXIT
    ENDIF

    '*********************************

О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_4: 'О©╫О©╫О©╫О©╫О©╫О©╫
    MOVE G6D,95,  95, 120, 100, 104
    MOVE G6A,102,  77, 147,  93,  102
    MOVE G6C, 85
    MOVE G6B,115
    WAIT


О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_5:
    MOVE G6D,103,    73, 140, 103,  100
    MOVE G6A, 95,  85, 147,  85, 102
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    ERX 4800,A, О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN
О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_5_stop:
        MOVE G6A,95,  90, 125, 95, 104
        MOVE G6D,104,  76, 145,  91,  102
        MOVE G6B, 100
        MOVE G6C,100
        WAIT

        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        'HIGHSPEED SETOFF
        GOTO RX_EXIT
    ENDIF
    '*************************************

    GOTO О©╫О©╫О©╫н╣О©╫О©╫О©╫О©╫О©╫О©╫_1

    '******************************************
О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50:
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    DELAY 50
    SPEED 6
    HIGHSPEED SETON



    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 145,  93, 101
        MOVE G6D,101,  77, 146,  93, 98
        WAIT

        MOVE G6A,95,  82, 120, 120, 104
        MOVE G6D,104,  79, 147,  91,  102
        MOVE G6B, 80
        MOVE G6C,120
        WAIT


        GOTO О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_2
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 146,  93, 101
        MOVE G6A,101,  77, 145,  93, 98
        WAIT

        MOVE G6D,95,  82, 121, 120, 104
        MOVE G6A,104,  79, 146,  91,  102
        MOVE G6C, 80
        MOVE G6B,120
        WAIT


        GOTO О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_5
    ENDIF


    '**********************

О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1:
    MOVE G6A,95,  97, 100, 120, 104
    MOVE G6D,104,  79, 148,  93,  102
    MOVE G6B, 80
    MOVE G6C,120
    WAIT


О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_2:
    MOVE G6A,95,  77, 122, 120, 104
    MOVE G6D,104,  80, 148,  90,  100
    WAIT

О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3:
    MOVE G6A,103,  69, 145, 103,  100
    MOVE G6D, 95, 87, 161,  68, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3_stop:

        MOVE G6D,90,  93, 116, 100, 104
        MOVE G6A,104,  74, 145,  91,  102
        MOVE G6C, 100
        MOVE G6B,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        DELAY 150
        GOTO RX_EXIT
    ENDIF
    '*********************************

О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_4:
    MOVE G6D,95,  97, 101, 120, 104
    MOVE G6A,104,  79, 147,  93,  102
    MOVE G6C, 80
    MOVE G6B,120
    WAIT


О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_5:
    MOVE G6D,95,  77, 123, 120, 104
    MOVE G6A,104,  80, 147,  90,  100
    WAIT


О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6:
    MOVE G6D,103,  71, 146, 103,  100
    MOVE G6A, 95, 89, 160,  68, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF
    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6_stop
    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6_stop:

        MOVE G6A,90,  93, 115, 100, 104
        MOVE G6D,104,  74, 146,  91,  102
        MOVE G6B, 100
        MOVE G6C,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        DELAY 150
        GOTO RX_EXIT
    ENDIF
    GOTO О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    О©╫О©╫О©╫О©╫COUNT = 0
    О©╫О©╫О©╫О©╫с╣О©╫ = 13
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    GOSUB Leg_motor_mode3

    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1

        SPEED 4

        MOVE G6A, 88,  74, 144,  95, 110
        MOVE G6D,108,  76, 148,  93,  96
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        SPEED 10'

        MOVE G6A, 90, 90, 120, 105, 110,100
        MOVE G6D,110,  76, 149,  93,  96,100
        MOVE G6B,90
        MOVE G6C,110
        WAIT


        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1	
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0

        SPEED 4

        MOVE G6D,  88,  74, 146,  95, 110
        MOVE G6A, 108,  76, 146,  93,  96
        MOVE G6C, 100
        MOVE G6B, 100
        WAIT

        SPEED 10

        MOVE G6D, 90, 90, 122, 105, 110,100
        MOVE G6A,110,  76, 147,  93,  96,100
        MOVE G6C,90
        MOVE G6B,110
        WAIT


        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2	

    ENDIF


    '*******************************



О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:

    ETX 4800,11 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫
    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6A, 86,  56, 145, 115, 110
    MOVE G6D,108,  76, 149,  93,  96
    WAIT


    SPEED О©╫б©О©╫с╣О©╫
    GOSUB Leg_motor_mode3

    MOVE G6A,110,  76, 147, 93,  96
    MOVE G6D,86, 100, 147,  69, 110
    WAIT


    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2
    ELSE
        ' GOSUB Leg_motor_mode3

        MOVE G6A,112,  76, 146,  93, 96,100
        MOVE G6D,90, 100, 102, 115, 110,100
        MOVE G6B,110
        MOVE G6C,90
        WAIT
        HIGHSPEED SETOFF

        SPEED 8
        MOVE G6A, 106,  76, 146,  93,  96,100		
        MOVE G6D,  88,  71, 154,  91, 106,100
        MOVE G6B, 100
        MOVE G6C, 100
        WAIT	

        SPEED 2
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        GOTO RX_EXIT
    ENDIF
    '**********

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:

    MOVE G6A,110,  76, 147,  93, 96,100
    MOVE G6D,90, 90, 122, 105, 110,100
    MOVE G6B,110
    MOVE G6C,90
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    ETX 4800,11 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 86,  56, 147, 115, 110
    MOVE G6A,108,  76, 147,  93,  96
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,110,  76, 149, 93,  96
    MOVE G6A,86, 100, 145,  69, 110
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ELSE

        MOVE G6A, 90, 100, 100, 115, 110,100
        MOVE G6D,112,  76, 148,  93,  96,100
        MOVE G6B,90
        MOVE G6C,110
        WAIT
        HIGHSPEED SETOFF
        SPEED 8

        MOVE G6D, 106,  76, 148,  93,  96,100		
        MOVE G6A,  88,  71, 152,  91, 106,100
        MOVE G6C, 100
        MOVE G6B, 100
        WAIT	
        SPEED 2
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        GOTO RX_EXIT
    ENDIF

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    'О©╫ч╧ъ╣О©╫О©╫10
    MOVE G6A,90, 90, 120, 105, 110,100
    MOVE G6D,110,  76, 148,  93,  96,100
    MOVE G6B, 90
    MOVE G6C,110
    WAIT

    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    '*******************************
    '*******************************


    '************************************************
О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫:
    О©╫О©╫О©╫О©╫с╣О©╫ = 12
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    MOVE G6A, 100,  76, 145,  93, 100, 100
    MOVE G6D, 100,  76, 145,  93, 100, 100
    MOVE G6B, 100,  30,  80, 100, 100, 102
    MOVE G6C, 100,  30,  80, 100,  89, 100
    WAIT


    GOSUB Leg_motor_mode3
    HIGHSPEED SETON
    SPEED 10
    MOVE G6D,  90,  74, 144,  95, 110
    MOVE G6A, 108,  76, 146,  93, 96
    MOVE G6C, 100
    MOVE G6B, 100
    WAIT

    SPEED 12
    MOVE G6D,90, 90, 120, 105, 110,100
    MOVE G6A,108,  76, 147,  93,  96,100
    MOVE G6C,90
    MOVE G6B,110
    WAIT

    HIGHSPEED SETOFF
    GOTO О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫_2	

О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫_2:
    MOVE G6A,110,  76, 147,  93, 100,100
    MOVE G6D,96, 90, 120, 102, 107,100
    MOVE G6B,110
    MOVE G6C,90
    WAIT

О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫_3:
    ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 90,  56, 145, 115, 112
    MOVE G6A,108,  76, 147,  93,  96
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,108,  76, 147, 93,  98
    MOVE G6A,90, 100, 145,  69, 108
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫_4
    IF A = 11 THEN
        GOTO О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫_4:
    SPEED 13
    MOVE G6A,95, 90, 120, 105, 111,100
    MOVE G6D,108,  76, 146,  93,  96,100
    MOVE G6B, 90
    MOVE G6C,110
    WAIT

    ' SPEED 10
    '  GOSUB О©╫Б╨╩О©╫з╪О©╫2
    ' GOTO О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫

    SPEED 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN

    '*******************************************************************************************************************************
О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫2:
    О©╫О©╫О©╫О©╫с╣О©╫ = 8
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    MOVE G6D, 100,  76, 145,  93, 100, 100
    MOVE G6A, 100,  76, 145,  93, 100, 100
    MOVE G6B, 100,  30,  80, 100, 100, 102
    MOVE G6C, 100,  30,  80, 100,  89, 100
    WAIT


    GOSUB Leg_motor_mode3
    HIGHSPEED SETON
    SPEED 10
    MOVE G6A,  90,  74, 144,  95, 110
    MOVE G6D, 108,  76, 146,  93, 96
    MOVE G6C, 100
    MOVE G6B, 100
    WAIT

    SPEED 12
    MOVE G6A,90, 90, 120, 105, 110,100
    MOVE G6D,108,  76, 147,  93,  96,100
    MOVE G6C,90
    MOVE G6B,110
    WAIT

    HIGHSPEED SETOFF
    GOTO О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫2_2	

О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫2_2:
    MOVE G6D,110,  76, 147,  93, 100,100
    MOVE G6A,96, 90, 120, 102, 107,100
    MOVE G6B,110
    MOVE G6C,90
    WAIT

О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫2_3:
    ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6A, 90,  56, 145, 115, 112
    MOVE G6D,108,  76, 147,  93,  96
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6A,108,  76, 147, 93,  98
    MOVE G6D,90, 100, 145,  69, 108
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫2_4
    IF A = 11 THEN
        GOTO О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫2_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫я╟О©╫О©╫О©╫О©╫х╠О©╫2_4:
    SPEED 9
    MOVE G6D,95, 90, 120, 105, 111,100
    MOVE G6A,108,  76, 146,  93,  96,100
    MOVE G6B, 90
    MOVE G6C,110
    WAIT

    SPEED 6
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN
    '*******************************************************
О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    MOVE G6B,185,  10,  60
    MOVE G6C,185,  10,  60
    WAIT

    HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 147,  93, 98
        MOVE G6D,101,  76, 146,  93, 95
        WAIT

        GOTO О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 146,  93, 98
        MOVE G6A,101,  76, 147,  93, 95
        WAIT

        GOTO О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF


    '**********************

О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    MOVE G6A,95,  90, 125, 100, 101
    MOVE G6D,104,  77, 146,  93,  99
    WAIT


О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:

    MOVE G6A,103,   73, 140, 103,  97
    MOVE G6D, 95,  85, 146,  85, 99
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop:
        MOVE G6D,95,  90, 124, 95, 101
        MOVE G6A,104,  76, 145,  91,  99
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        MOVE G6A,98,  76, 145,  93, 101, 97
        MOVE G6D,98,  76, 145,  93, 101, 97
        'GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        '  SPEED 5
        ' GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*********************************

О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    MOVE G6D,95,  95, 119, 100, 101
    MOVE G6A,104,  77, 147,  93,  99
    WAIT


О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5:
    MOVE G6D,103,    73, 139, 103,  97
    MOVE G6A, 95,  85, 147,  85, 99
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop:
        MOVE G6A,95,  90, 125, 95, 101
        MOVE G6D,104,  76, 144,  91,  99
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        MOVE G6A,98,  76, 145,  93, 101, 97
        MOVE G6D,98,  76, 145,  93, 101, 97
        ' GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        ' SPEED 5
        '  GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*************************************

    '*********************************

    GOTO О©╫О©╫О©╫О©╫х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1

    '-----------------------------------------------------------------

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    О©╫О©╫О©╫О©╫с╣О©╫ = 12
    О©╫б©О©╫с╣О©╫ = 4
    GOSUB Leg_motor_mode3



    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1

        SPEED 4
        MOVE G6A, 88,  71, 152,  91, 110
        MOVE G6D,108,  76, 145,  93,  96
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        SPEED 10
        MOVE G6A, 90, 100, 100, 115, 110
        MOVE G6D,110,  76, 145,  93,  96
        MOVE G6B,90
        MOVE G6C,110
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1	
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0

        SPEED 4
        MOVE G6D,  88,  71, 152,  91, 110
        MOVE G6A, 108,  76, 145,  93,  96
        MOVE G6C, 100
        MOVE G6B, 100
        WAIT

        SPEED 10
        MOVE G6D, 90, 100, 100, 115, 110
        MOVE G6A,110,  76, 145,  93,  96
        MOVE G6C,90
        MOVE G6B,110
        WAIT


        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2

    ENDIF


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    ETX 4800,12 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫
    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D,110,  76, 145, 93,  96
    MOVE G6A,90, 98, 145,  69, 110
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D, 90,  60, 137, 120, 110
    MOVE G6A,107,  85, 137,  93,  96
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    SPEED 11

    MOVE G6D,90, 90, 120, 105, 110
    MOVE G6A,112,  76, 146,  93, 96
    MOVE G6B,110
    MOVE G6C,90
    WAIT

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1_EXIT:
        HIGHSPEED SETOFF
        SPEED 5

        MOVE G6A, 106,  76, 145,  93,  96		
        MOVE G6D,  85,  72, 148,  91, 106
        MOVE G6B, 100
        MOVE G6C, 100
        WAIT	

        SPEED 3
        GOSUB О©╫Б╨╩О©╫з╪О©╫2
        GOTO RX_EXIT
    ENDIF
    '**********

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:
    ETX 4800,12 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫
    SPEED О©╫О©╫О©╫О©╫с╣О©╫
    MOVE G6A,110,  76, 145, 93,  96
    MOVE G6D,90, 98, 145,  69, 110
    WAIT


    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6A, 90,  60, 137, 120, 110
    MOVE G6D,107  85, 137,  93,  96
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    SPEED 11
    MOVE G6A,90, 90, 120, 105, 110
    MOVE G6D,112,  76, 146,  93,  96
    MOVE G6B, 90
    MOVE G6C,110
    WAIT


    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_EXIT:
        HIGHSPEED SETOFF
        SPEED 5

        MOVE G6D, 106,  76, 145,  93,  96		
        MOVE G6A,  85,  72, 148,  91, 106
        MOVE G6B, 100
        MOVE G6C, 100
        WAIT	

        SPEED 3
        GOSUB О©╫Б╨╩О©╫з╪О©╫2
        GOTO RX_EXIT
    ENDIF  	

    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    '**********************************************

    '******************************************
х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 147,  93, 101
        MOVE G6D,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 147,  93, 101
        MOVE G6A,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF


    '**********************

х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    MOVE G6A,95,  90, 125, 100, 104
    MOVE G6D,104,  77, 147,  93,  102
    MOVE G6B, 85
    MOVE G6C,115
    WAIT


х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:

    MOVE G6A,103,   73, 140, 103,  100
    MOVE G6D, 95,  85, 147,  85, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop

    ERX 4800,A, х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop:
        MOVE G6D,95,  90, 125, 95, 104
        MOVE G6A,104,  76, 145,  91,  102
        MOVE G6C, 100
        MOVE G6B,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*********************************

х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    MOVE G6D,95,  95, 120, 100, 104
    MOVE G6A,104,  77, 147,  93,  102
    MOVE G6C, 85
    MOVE G6B,115
    WAIT


х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5:
    MOVE G6D,103,    73, 140, 103,  100
    MOVE G6A, 95,  85, 147,  85, 102
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop

    ERX 4800,A, х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN
х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop:
        MOVE G6A,95,  90, 125, 95, 104
        MOVE G6D,104,  76, 145,  91,  102
        MOVE G6B, 100
        MOVE G6C,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*************************************

    '*********************************

    GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1

    '******************************************

    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 147,  93, 101
        MOVE G6D,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 147,  93, 101
        MOVE G6A,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF


    '**********************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    MOVE G6A,95,  90, 125, 100, 104
    MOVE G6D,104,  77, 147,  93,  102
    MOVE G6B, 85
    MOVE G6C,115
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:

    MOVE G6A,103,   73, 140, 103,  100
    MOVE G6D, 95,  85, 147,  85, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

        GOTO RX_EXIT
    ENDIF

    ' О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    'IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop:
        MOVE G6D,95,  90, 125, 95, 104
        MOVE G6A,104,  76, 145,  91,  102
        MOVE G6C, 100
        MOVE G6B,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*********************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    MOVE G6D,95,  95, 120, 100, 104
    MOVE G6A,104,  77, 147,  93,  102
    MOVE G6C, 85
    MOVE G6B,115
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5:
    MOVE G6D,103,    73, 140, 103,  100
    MOVE G6A, 95,  85, 147,  85, 102
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    ' О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    ' IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop:
        MOVE G6A,95,  90, 125, 95, 104
        MOVE G6D,104,  76, 145,  91,  102
        MOVE G6B, 100
        MOVE G6C,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

О©╫х╠О©╫:


    '*************************************
    
    

    '*********************************

    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1

    '******************************************
    '******************************************
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 145,  93, 101
        MOVE G6D,101,  76, 145,  93, 98
        MOVE G6B,101
        MOVE G6C,101
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 145,  93, 101
        MOVE G6A,101,  76, 145,  93, 98
        MOVE G6B,101
        MOVE G6C,101
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF


    '**********************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    MOVE G6D,104,  76, 147,  93,  102
    MOVE G6A,95,  95, 120, 95, 104
    MOVE G6B,116
    MOVE G6C,86
    WAIT



О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    MOVE G6A, 103,  79, 147,  89, 100
    MOVE G6D,95,   65, 147, 103,  102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF
    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3_stop
    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3_stop:
        MOVE G6D,95,  85, 130, 100, 104
        MOVE G6A,104,  77, 146,  93,  102
        MOVE G6C, 101
        MOVE G6B,101
        WAIT

        'SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        HIGHSPEED SETOFF
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF
    '*********************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    MOVE G6A,104,  76, 147,  93,  102
    MOVE G6D,95,  95, 120, 95, 104
    MOVE G6C,116
    MOVE G6B,86
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6:
    MOVE G6D, 103,  79, 147,  89, 100
    MOVE G6A,95,   65, 147, 103,  102
    WAIT
    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN  'GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_О©╫О©╫О©╫О©╫
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6_stop:
        MOVE G6A,95,  85, 130, 100, 104
        MOVE G6D,104,  77, 146,  93,  102
        MOVE G6B, 101
        MOVE G6C,101
        WAIT

        'SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        HIGHSPEED SETOFF
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1




    '******************************************


    '******************************************
    '******************************************

    '******************************************
    '*************************************

    '******************************************
О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    О©╫Н╪╠О©╫О©╫О©╫О©╫ = 2
    SPEED 10
    HIGHSPEED SETON
    GOSUB All_motor_mode3


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 145,  93, 101
        MOVE G6D,101,  77, 145,  93, 98
        MOVE G6B,100,  35
        MOVE G6C,100,  35
        WAIT

        GOTO О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 145,  93, 101
        MOVE G6A,101,  77, 145,  93, 98
        MOVE G6B,100,  35
        MOVE G6C,100,  35
        WAIT

        GOTO О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF



    '**********************

О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    SPEED 8
    MOVE G6A,95,  95, 120, 100, 104
    MOVE G6D,104,  77, 146,  91,  102
    MOVE G6B, 85
    MOVE G6C,115
    WAIT


О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    SPEED 8
    MOVE G6A,103,   71, 140, 105,  100
    MOVE G6D, 95,  82, 146,  87, 102
    WAIT


    ERX 4800, A ,О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4_0

    IF A = 20 THEN
        О©╫Н╪╠О©╫О©╫О©╫О©╫ = 3
    ELSEIF A = 43 THEN
        О©╫Н╪╠О©╫О©╫О©╫О©╫ = 1
    ELSEIF A = 11 THEN
        О©╫Н╪╠О©╫О©╫О©╫О©╫ = 2
    ELSE  'О©╫О©╫О©╫О©╫
        GOTO О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3О©╫О©╫О©╫О©╫
    ENDIF

О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4_0:

    IF  О©╫Н╪╠О©╫О©╫О©╫О©╫ = 1 THEN'О©╫О©╫О©╫О©╫

    ELSEIF  О©╫Н╪╠О©╫О©╫О©╫О©╫ = 3 THEN'О©╫О©╫О©╫О©╫О©╫О©╫
        HIGHSPEED SETOFF
        SPEED 8
        MOVE G6D,103,   71, 140, 105,  100
        MOVE G6A, 95,  82, 146,  87, 102
        WAIT
        HIGHSPEED SETON
        GOTO О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1

    ENDIF



    '*********************************

О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    SPEED 8
    MOVE G6D,95,  95, 120, 100, 104
    MOVE G6A,104,  77, 146,  91,  102
    MOVE G6C, 85
    MOVE G6B,115
    WAIT


О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6:
    SPEED 8
    MOVE G6D,103,   71, 140, 105,  100
    MOVE G6A, 95,  82, 146,  87, 102
    WAIT



    ERX 4800, A ,О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1_0

    IF A = 20 THEN
        О©╫Н╪╠О©╫О©╫О©╫О©╫ = 3
    ELSEIF A = 43 THEN
        О©╫Н╪╠О©╫О©╫О©╫О©╫ = 1
    ELSEIF A = 11 THEN
        О©╫Н╪╠О©╫О©╫О©╫О©╫ = 2
    ELSE  'О©╫О©╫О©╫О©╫
        GOTO О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6О©╫О©╫О©╫О©╫
    ENDIF

О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1_0:

    IF  О©╫Н╪╠О©╫О©╫О©╫О©╫ = 1 THEN'О©╫О©╫О©╫О©╫
        HIGHSPEED SETOFF
        SPEED 8
        MOVE G6A,103,   71, 140, 105,  100
        MOVE G6D, 95,  82, 146,  87, 102
        WAIT
        HIGHSPEED SETON
        GOTO О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ELSEIF О©╫Н╪╠О©╫О©╫О©╫О©╫ = 3 THEN'О©╫О©╫О©╫О©╫О©╫О©╫


    ENDIF



    GOTO О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    '******************************************
    '******************************************
    '*********************************
О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3О©╫О©╫О©╫О©╫:
    MOVE G6D,95,  90, 125, 95, 104
    MOVE G6A,104,  76, 145,  91,  102
    MOVE G6C, 100
    MOVE G6B,100
    WAIT
    HIGHSPEED SETOFF
    SPEED 15
    GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
    SPEED 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫
    GOTO MAIN	
    '******************************************
О©╫Н╪╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6О©╫О©╫О©╫О©╫:
    MOVE G6A,95,  90, 125, 95, 104
    MOVE G6D,104,  76, 145,  91,  102
    MOVE G6B, 100
    MOVE G6C,100
    WAIT
    HIGHSPEED SETOFF
    SPEED 15
    GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
    SPEED 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫
    GOTO MAIN	
    '******************************************

    '************************************************
О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫20: '****
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

    SPEED 12
    MOVE G6D, 95,  90, 125, 100, 109, 100
    MOVE G6A,105,  78, 146,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  77, 145, 93, 100, 100
    MOVE G6A,90,  82, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6D,95,  76, 145,  93, 102, 100
    MOVE G6A,95,  78, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  78, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    GOSUB All_motor_mode3
    GOTO RX_EXIT
    '*************
    '*************

О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫20: '****
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

    SPEED 12
    MOVE G6A, 95,  90, 125, 100, 109, 100 ' before 95, 90, 125, 100, 104, 100
    MOVE G6D,105,  78, 145,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6A, 102,  77, 145, 93, 100, 100
    MOVE G6D,90,  82, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6A,95,  76, 145,  93, 102, 100
    MOVE G6D,95,  78, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  78, 145,  93, 100, 100
    GOSUB All_motor_mode3
    GOTO RX_EXIT

    '**********************************************
    '******************************************
О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop:
    DELAY  10

    SPEED 10
    MOVE G6D, 90,  90, 120, 105, 110, 100
    MOVE G6A,100,  76, 145,  93, 107, 100
    'MOVE G6C,100,  40
    'MOVE G6B,100,  40
    WAIT

    SPEED 13
    MOVE G6D, 102,  76, 145, 95, 100, 100
    MOVE G6A,83,  78, 140,  98, 115, 100
    WAIT

    SPEED 13
    MOVE G6D,98,  76, 145,  95, 100, 100
    MOVE G6A,98,  76, 145,  95, 100, 100
    WAIT

    SPEED 12
    MOVE G6A,100,  76, 145,  95, 100, 100
    MOVE G6D,100,  76, 145,  95, 100, 100
    WAIT


    '  ERX 4800, A ,О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop
    '    IF A = A_OLD THEN  GOTO О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop
    'О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_stop:
    GOSUB О©╫Б╨╩О©╫з╪О©╫2

    GOTO RX_EXIT
    '**********************************************

О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop:
    DELAY  10

    SPEED 10
    MOVE G6A, 90,  90, 120, 105, 110, 100	
    MOVE G6D,100,  76, 145,  93, 107, 100	
    'MOVE G6C,100,  40
    'MOVE G6B,100,  40
    WAIT

    SPEED 13
    MOVE G6A, 102,  76, 145, 95, 100, 100
    MOVE G6D,83,  78, 140,  98, 115, 100
    WAIT

    SPEED 13
    MOVE G6A,98,  76, 145,  95, 100, 100
    MOVE G6D,98,  76, 145,  95, 100, 100
    WAIT

    SPEED 12
    MOVE G6D,100,  76, 145,  95, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    WAIT

    '   ERX 4800, A ,О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop	
    '    IF A = A_OLD THEN  GOTO О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop
    'О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_stop:

    GOSUB О©╫Б╨╩О©╫з╪О©╫2

    GOTO RX_EXIT

    '**********************************************
    '************************************************
<<<<<<< HEAD
    '*********************************************

    'О©╫О©╫О©╫О©╫ О©╫О©╫О©╫о╟О©╫ О©╫О©╫О©╫О©╫О©╫О©╫ О©╫О©╫О©╫О©╫ О©╫т╪О©╫(О©╫О©╫О©╫О©╫О©╫О©╫ О©╫О©╫О©╫)
О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫н╟х╠О©╫:

    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    GOSUB All_motor_Reset
    DELAY  10

    SPEED 7
    MOVE G6D, 90,  90, 120, 105, 110, 100
    MOVE G6A,100,  76, 145,  93, 107, 100
    'MOVE G6C,100,  40
    'MOVE G6B,100,  40
    WAIT

    SPEED 13
    MOVE G6D, 102,  76, 145, 93, 100, 100
    MOVE G6A,83,  78, 140,  96, 115, 100
    WAIT

    SPEED 13
    MOVE G6D,98,  76, 145,  93, 100, 100
    MOVE G6A,98,  76, 145,  93, 100, 100
    WAIT

    SPEED 12
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT


    SPEED 5

    RETURN


О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫н╟х╠О©╫:

    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    GOSUB All_motor_Reset
    DELAY  10

    SPEED 7
    MOVE G6A, 90,  90, 120, 105, 110, 100
    MOVE G6D,100,  76, 145,  93, 107, 100
    'MOVE G6C,100,  40
    'MOVE G6B,100,  40
    WAIT

    SPEED 13
    MOVE G6A, 102,  76, 145, 93, 100, 100
    MOVE G6D,83,  78, 140,  96, 115, 100
    WAIT

    SPEED 13
    MOVE G6A,98,  76, 145,  93, 100, 100
    MOVE G6D,98,  76, 145,  93, 100, 100
    WAIT

    SPEED 12
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    WAIT


    SPEED 5

    RETURN

О©╫О©╫О©╫О©╫О©╫Б╣©О©╫О©╫:
    SPEED 5

    MOVE G6C,130,  90, 10
    MOVE G6B,100, 190 , 10	
    WAIT

    DELAY 200

    RETURN


О©╫О©╫О©╫О©╫О©╫Б╣©О©╫О©╫2:
    SPEED 5

    MOVE G6B,130,  90, 10
    MOVE G6C,100, 190 , 10	
    WAIT

    DELAY 200

    RETURN
    '*******************************
О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫:
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  77, 147,  93, 101
        MOVE G6D,101,  76, 149,  93, 98
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 149,  93, 101
        MOVE G6A,101,  77, 147,  93, 98
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_4
    ENDIF


    '**********************

О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1:
    MOVE G6A,95,  91, 125, 100, 104
    MOVE G6D,104,  77, 149,  93,  102
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2:

    MOVE G6A,103,   74, 140, 103,  100
    MOVE G6D, 95,  85, 149,  85, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_stop:
        MOVE G6D,95,  90, 127, 95, 104
        MOVE G6A,104,  77, 145,  91,  102
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        'GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        ' SPEED 5
        '  GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  75, 145,  93, 100, 100
        MOVE G6D,100,  76, 145,  93, 100, 100

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*********************************

О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_4:
    MOVE G6D,95,  96, 122, 100, 104
    MOVE G6A,104,  78, 147,  93,  102
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_5:
    MOVE G6D,103,    74, 142, 103,  100
    MOVE G6A, 95,  86, 147,  85, 102
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_5_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_5_stop:
        MOVE G6A,95,  91, 125, 95, 104
        MOVE G6D,104,  79, 145,  91,  102
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        '   GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        '  SPEED 5
        ' GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  75, 145,  93, 100, 100
        MOVE G6D,100,  77, 145,  93, 100, 100

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*************************************

    '*********************************

    GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1

О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    'HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  77, 145,  93, 101
        MOVE G6D,101,  76, 145,  93, 98
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 145,  93, 101
        MOVE G6A,101,  76, 145,  93, 98
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_4
    ENDIF


    '**********************

О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_1:
    MOVE G6D,104,  76, 147,  93,  102
    MOVE G6A,95,  95, 120, 95, 104
    WAIT



О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_3:
    MOVE G6A, 103,  79, 147,  89, 100
    MOVE G6D,95,   65, 147, 103,  102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF
    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_3_stop
    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_3_stop:
        MOVE G6D,95,  85, 130, 100, 104
        MOVE G6A,104,  77, 146,  93,  102
        WAIT

        'SPEED 15
        '        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        '       HIGHSPEED SETOFF
        '      SPEED 5
        '     GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  76, 145,  93, 100, 100
        MOVE G6D,100,  76, 145,  93, 100, 100

        '   DELAY 400
        GOTO RX_EXIT
    ENDIF
    '*********************************

О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_4:
    MOVE G6A,104,  76, 147,  93,  102
    MOVE G6D,95,  95, 120, 95, 104
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_6:
    MOVE G6D, 103,  79, 147,  89, 100
    MOVE G6A,95,   65, 147, 103,  102
    WAIT
    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_6_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN  'GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_О©╫О©╫О©╫О©╫
О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_6_stop:
        MOVE G6A,95,  85, 130, 100, 104
        MOVE G6D,104,  77, 146,  93,  102
        WAIT

        'SPEED 15
        '        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        '       HIGHSPEED SETOFF
        '      SPEED 5
        '     GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  76, 145,  93, 100, 100
        MOVE G6D,100,  76, 145,  93, 100, 100

        '  DELAY 400
        GOTO RX_EXIT
    ENDIF

    GOTO О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫О©╫О©╫О©╫О©╫_1

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP:
    'SPEED 5

    'MOVE G6B,130,  90, 10
    'MOVE G6C,100, 190 , 10	
    'WAIT

    ' DELAY 150
    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 15
        MOVE G6D,100,  73, 145,  91, 100, 100
        MOVE G6A,100,  79, 145,  91, 100, 100
        WAIT

        SPEED 6
        MOVE G6D,100,  84, 145,  76, 100, 100
        MOVE G6A,100,  68, 145,  106, 100, 100
        WAIT

        SPEED 9
        MOVE G6D,90,  90, 145,  76, 102, 100
        MOVE G6A,104,  71, 145,  103, 100, 100
        WAIT
        SPEED 7
        MOVE G6D,90,  80, 128, 102, 104
        MOVE G6A,105,  76, 144,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 15
        MOVE G6D,100,  73, 145,  91, 100, 100
        MOVE G6A,100,  79, 145,  91, 100, 100
        WAIT


        SPEED 6
        MOVE G6D,100,  88, 145,  76, 100, 100
        MOVE G6A,100,  65, 145,  106, 100, 100
        WAIT

        SPEED 9
        MOVE G6D,104,  86, 146,  78, 100, 100
        MOVE G6A,90,  58, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6A,90,  85, 128, 98, 104
        MOVE G6D,105,  77, 144,  93,  100
        WAIT


    ENDIF

    SPEED 12
    ' GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  91, 100, 100
    MOVE G6D,100,  76, 145,  91, 100, 100


    GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP:
    '   MOVE G6C,130,  90, 10
    '  MOVE G6B,100, 190 , 10	
    ' WAIT
    'DELAY 150
    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 15
        MOVE G6A,100,  73, 145,  91, 100, 100
        MOVE G6D,98,  79, 146,  91, 100, 100
        WAIT


        SPEED 6
        MOVE G6A,100,  84, 145,  76, 100, 100
        MOVE G6D,98,  68, 146,  106, 100, 100
        WAIT

        SPEED 9
        MOVE G6A,90,  90, 145,  76, 102, 100
        MOVE G6D,102  71, 146,  103, 100, 100
        WAIT
        SPEED 7
        MOVE G6A,90,  80, 128, 102, 104
        MOVE G6D,103,  76, 145,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 15
        MOVE G6A,100,  73, 145,  91, 100, 100
        MOVE G6D,98,  79, 146,  91, 100, 100
        WAIT


        SPEED 6
        MOVE G6A,100,  88, 145,  76, 100, 100
        MOVE G6D,98,  65, 146,  106, 100, 100
        WAIT

        SPEED 9
        MOVE G6A,104,  86, 146,  78, 100, 100
        MOVE G6D,88,  58, 146,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6D,88,  85, 129, 98, 104
        MOVE G6A,105,  77, 143,  93,  100
        WAIT

    ENDIF
    SPEED 12
    '  GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  91, 100, 100
    MOVE G6D,100,  76, 145,  91, 100, 100

    GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP_О©╫О©╫О©╫О©╫:
    SPEED 5

    MOVE G6C,130,  90, 10
    MOVE G6B,100, 190 , 10	
    WAIT

    DELAY 150
    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 15
        MOVE G6D,100,  73, 145,  93, 100, 100
        MOVE G6A,100,  79, 145,  93, 100, 100
        WAIT

        SPEED 6
        MOVE G6D,100,  84, 145,  78, 100, 100
        MOVE G6A,100,  68, 145,  108, 100, 100
        WAIT

        SPEED 9
        MOVE G6D,90,  90, 145,  78, 102, 100
        MOVE G6A,104,  71, 145,  105, 100, 100
        WAIT
        SPEED 7
        MOVE G6D,90,  80, 130, 102, 104
        MOVE G6A,105,  76, 146,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 15
        MOVE G6D,100,  73, 145,  93, 100, 100
        MOVE G6A,100,  79, 145,  93, 100, 100
        WAIT


        SPEED 6
        MOVE G6D,100,  88, 145,  78, 100, 100
        MOVE G6A,100,  65, 145,  108, 100, 100
        WAIT

        SPEED 9
        MOVE G6D,104,  86, 146,  80, 100, 100
        MOVE G6A,90,  58, 145,  110, 100, 100
        WAIT

        SPEED 7
        MOVE G6A,90,  85, 130, 98, 104
        MOVE G6D,105,  77, 146,  93,  100
        WAIT



    ENDIF

    SPEED 12
    ' GOSUB О©╫Б╨╩О©╫з╪О©╫2


    GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP_О©╫О©╫О©╫О©╫:
    '  SPEED 5

    MOVE G6C,130,  90, 10
    MOVE G6B,100, 190 , 10	
    WAIT

    DELAY 150
    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 15
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,98,  79, 146,  93, 100, 100
        WAIT


        SPEED 6
        MOVE G6A,100,  84, 145,  78, 100, 100
        MOVE G6D,98,  68, 146,  108, 100, 100
        WAIT

        SPEED 9
        MOVE G6A,90,  90, 145,  78, 102, 100
        MOVE G6D,102  71, 146,  105, 100, 100
        WAIT
        SPEED 7
        MOVE G6A,90,  80, 130, 102, 104
        MOVE G6D,103,  76, 147,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 15
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,98,  79, 146,  93, 100, 100
        WAIT


        SPEED 6
        MOVE G6A,100,  88, 145,  78, 100, 100
        MOVE G6D,98,  65, 146,  108, 100, 100
        WAIT

        SPEED 9
        MOVE G6A,104,  86, 146,  80, 100, 100
        MOVE G6D,88,  58, 146,  110, 100, 100
        WAIT

        SPEED 7
        MOVE G6D,88,  85, 131, 98, 104
        MOVE G6A,105,  77, 145,  93,  100
        WAIT

    ENDIF
    SPEED 12
    '  GOSUB О©╫Б╨╩О©╫з╪О©╫2

    GOTO RX_EXIT


=======
>>>>>>> e888a3d (Chore: К╛╦ К╙╗Л┘≤ Л┌╜Л═°)

О©╫О©╫О©╫О©╫О©╫О©╫3:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP:

    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 17
        MOVE G6D,100,  73, 145,  93, 100, 100
        MOVE G6A,100,  79, 145,  93, 100, 100
        WAIT

        SPEED 8
        MOVE G6D,100,  84, 145,  78, 100, 100
        MOVE G6A,100,  68, 145,  108, 100, 100
        WAIT

        SPEED 11
        MOVE G6D,90,  90, 145,  78, 102, 100
        MOVE G6A,104,  71, 145,  105, 100, 100
        WAIT
        SPEED 9
        MOVE G6D,90,  80, 130, 102, 104
        MOVE G6A,105,  76, 146,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 17
        MOVE G6D,100,  73, 145,  93, 100, 100
        MOVE G6A,100,  79, 145,  93, 100, 100
        WAIT


        SPEED 8
        MOVE G6D,100,  88, 145,  78, 100, 100
        MOVE G6A,100,  65, 145,  108, 100, 100
        WAIT

        SPEED 11
        MOVE G6D,104,  86, 146,  80, 100, 100
        MOVE G6A,90,  58, 145,  110, 100, 100
        WAIT

        SPEED 9
        MOVE G6A,90,  85, 130, 98, 104
        MOVE G6D,105,  77, 146,  93,  100
        WAIT



    ENDIF

    SPEED 14
    GOSUB О©╫Б╨╩О©╫з╪О©╫2


    GOTO RX_EXIT

    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP:

    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 17
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,98,  79, 146,  93, 100, 100
        WAIT


        SPEED 8
        MOVE G6A,100,  84, 145,  78, 100, 100
        MOVE G6D,98,  68, 146,  108, 100, 100
        WAIT

        SPEED 11
        MOVE G6A,90,  90, 145,  78, 102, 100
        MOVE G6D,102  71, 146,  105, 100, 100
        WAIT
        SPEED 9
        MOVE G6A,90,  80, 130, 102, 104
        MOVE G6D,103,  76, 147,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 15
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,98,  79, 146,  93, 100, 100
        WAIT


        SPEED 8
        MOVE G6A,100,  88, 145,  78, 100, 100
        MOVE G6D,98,  65, 146,  108, 100, 100
        WAIT

        SPEED 11
        MOVE G6A,104,  86, 146,  80, 100, 100
        MOVE G6D,88,  58, 146,  110, 100, 100
        WAIT

        SPEED 9
        MOVE G6D,88,  85, 131, 98, 104
        MOVE G6A,105,  77, 145,  93,  100
        WAIT

    ENDIF
    SPEED 14
    GOSUB О©╫Б╨╩О©╫з╪О©╫2

    GOTO RX_EXIT

    '******************************************************
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫10:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 5
    MOVE G6A,97,  86, 145,  83, 103, 100
    MOVE G6D,97,  66, 145,  103, 103, 100
    WAIT

    SPEED 12
    MOVE G6A,94,  86, 145,  83, 101, 100
    MOVE G6D,94,  66, 145,  103, 101, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    WAIT

    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫10:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 5
    MOVE G6A,97,  66, 145,  103, 103, 100
    MOVE G6D,97,  86, 145,  83, 103, 100
    WAIT

    SPEED 12
    MOVE G6A,94,  66, 145,  103, 101, 100
    MOVE G6D,94,  86, 145,  83, 101, 100
    WAIT
    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    WAIT

    GOSUB О©╫Б╨╩О©╫з╪О©╫2

    GOTO RX_EXIT
    '**********************************************
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫20:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 8
    MOVE G6A,95,  96, 145,  73, 105, 100
    MOVE G6D,95,  56, 145,  113, 105, 100
    MOVE G6B,110
    MOVE G6C,90
    WAIT

    SPEED 12
    MOVE G6A,93,  96, 145,  73, 105, 100
    MOVE G6D,93,  56, 145,  113, 105, 100
    WAIT
    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100

    WAIT

    GOSUB О©╫Б╨╩О©╫з╪О©╫2

    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫20:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 8
    MOVE G6A,95,  56, 145,  113, 105, 100
    MOVE G6D,95,  96, 145,  73, 105, 100
    MOVE G6B,90
    MOVE G6C,110
    WAIT

    SPEED 12
    MOVE G6A,93,  56, 145,  113, 105, 100
    MOVE G6D,93,  96, 145,  73, 105, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100

    WAIT

    GOSUB О©╫Б╨╩О©╫з╪О©╫2

    GOTO RX_EXIT
    '**********************************************
<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫20:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 8
    MOVE G6A,95,  56, 145,  113, 105, 100
    MOVE G6D,95,  96, 145,  73, 105, 100
    '  MOVE G6B,90
    ' MOVE G6C,110
    WAIT

    SPEED 12
    MOVE G6A,93,  56, 145,  113, 105, 100
    MOVE G6D,93,  96, 145,  73, 105, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100

    WAIT

    '  GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOTO RX_EXIT
    '**********************************************	

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫20:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 8
    MOVE G6A,95,  96, 145,  73, 105, 100
    MOVE G6D,95,  56, 145,  113, 105, 100
    'MOVE G6B,110
    'MOVE G6C,90
    WAIT

    SPEED 12
    MOVE G6A,93,  96, 145,  73, 105, 100
    MOVE G6D,93,  56, 145,  113, 105, 100
    WAIT
    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100

    WAIT

    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 10
    MOVE G6A,95, 106, 145,  63, 105, 100
    MOVE G6D,95,  46, 145,  123, 105, 100
    'MOVE G6B,110
    'MOVE G6C,90
    WAIT

    SPEED 12
    MOVE G6A,93,  106, 145,  63, 105, 100
    MOVE G6D,93,  46, 145,  123, 105, 100
    WAIT
    SPEED 8
    '   MOVE G6A,101,  76, 146,  93, 98, 100
    '  MOVE G6D,101,  76, 146,  93, 98, 100

    '  WAIT

    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    SPEED 10
    MOVE G6A,95,  46, 145,  123, 105, 100
    MOVE G6D,95,  106, 145,  63, 105, 100
    '  MOVE G6B,90
    ' MOVE G6C,110
    WAIT

    SPEED 12
    MOVE G6A,93,  46, 145,  123, 105, 100
    MOVE G6D,93,  106, 145,  63, 105, 100
    WAIT

    SPEED 8
    ' MOVE G6A,101,  76, 146,  93, 98, 100
    'MOVE G6D,101,  76, 146,  93, 98, 100

    'WAIT

    '  GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫45:
=======
©чбйео45:
>>>>>>> e888a3d (Chore: К╛╦ К╙╗Л┘≤ Л┌╜Л═°)
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
О©╫О©╫О©╫О©╫О©╫О©╫45_LOOP:

    SPEED 10
    MOVE G6A,95,  106, 145,  63, 105, 100
    MOVE G6D,95,  46, 145,  123, 105, 100
    WAIT

    SPEED 12
    MOVE G6A,93,  106, 145,  63, 105, 100
    MOVE G6D,93,  46, 145,  123, 105, 100
    WAIT

    SPEED 8
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    'DELAY 50
    '    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    '    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
    '        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    '        GOTO RX_EXIT
    '    ENDIF
    '
    '    ERX 4800,A,О©╫О©╫О©╫О©╫О©╫О©╫45_LOOP
    '    IF A_old = A THEN GOTO О©╫О©╫О©╫О©╫О©╫О©╫45_LOOP
    '
    GOTO RX_EXIT

    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45_LOOP:

    SPEED 10
    MOVE G6A,95,  46, 145,  123, 105, 100
    MOVE G6D,95,  106, 145,  63, 105, 100
    WAIT

    SPEED 12
    MOVE G6A,93,  46, 145,  123, 105, 100
    MOVE G6D,93,  106, 145,  63, 105, 100
    WAIT

    SPEED 8
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    ' DELAY 50
    '    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    '    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
    '        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    '        GOTO RX_EXIT
    '    ENDIF
    '
    '    ERX 4800,A,О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45_LOOP
    '    IF A_old = A THEN GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45_LOOP
    '
    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫60:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
О©╫О©╫О©╫О©╫О©╫О©╫60_LOOP:

    SPEED 15
    MOVE G6A,95,  116, 145,  53, 105, 100
    MOVE G6D,95,  36, 145,  133, 105, 100
    WAIT

    SPEED 15
    MOVE G6A,90,  116, 145,  53, 105, 100
    MOVE G6D,90,  36, 145,  133, 105, 100
    WAIT

    SPEED 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    '  DELAY 50
    '    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    '    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
    '        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    '        GOTO RX_EXIT
    '    ENDIF
    '    ERX 4800,A,О©╫О©╫О©╫О©╫О©╫О©╫60_LOOP
    '    IF A_old = A THEN GOTO О©╫О©╫О©╫О©╫О©╫О©╫60_LOOP

    GOTO RX_EXIT

    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫60:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫60_LOOP:

    SPEED 15
    MOVE G6A,95,  36, 145,  133, 105, 100
    MOVE G6D,95,  116, 145,  53, 105, 100
    WAIT

    SPEED 15
    MOVE G6A,90,  36, 145,  133, 105, 100
    MOVE G6D,90,  116, 145,  53, 105, 100

    WAIT

    SPEED 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    ' DELAY 50
    '    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    '    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
    '        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    '        GOTO RX_EXIT
    '    ENDIF
    '    ERX 4800,A,О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫60_LOOP
    '    IF A_old = A THEN GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫60_LOOP

    GOTO RX_EXIT
    '****************************************
    '************************************************
    '**********************************************


    '************************************************

    ''************************************************
    '************************************************
    '************************************************
О©╫з╥О©╫О©╫о╬НЁ╙О©╫О©╫:

    HIGHSPEED SETOFF
    PTP SETON 				
    PTP ALLON		

    GOSUB О©╫О©╫О©╫л╥О©╫OFF

    GOSUB All_motor_Reset

    SPEED 15
    GOSUB О©╫Б╨╩О©╫з╪О©╫

    MOVE G6A,90, 130, ,  80, 110, 100
    MOVE G6D,90, 130, 120,  80, 110, 100
    MOVE G6B,150, 160,  10, 100, 100, 100
    MOVE G6C,150, 160,  10, 100, 100, 100
    WAIT

    MOVE G6B,170, 140,  10, 100, 100, 100
    MOVE G6C,170, 140,  10, 100, 100, 100
    WAIT

    MOVE G6B,185,  20, 70,  100, 100, 100
    MOVE G6C,185,  20, 70,  100, 100, 100
    WAIT
    SPEED 10
    MOVE G6A, 80, 155,  85, 150, 150, 100
    MOVE G6D, 80, 155,  85, 150, 150, 100
    MOVE G6B,185,  20, 70,  100, 100, 100
    MOVE G6C,185,  20, 70,  100, 100, 100
    WAIT



    MOVE G6A, 75, 162,  55, 162, 155, 100
    MOVE G6D, 75, 162,  59, 162, 155, 100
    MOVE G6B,188,  10, 100, 100, 100, 100
    MOVE G6C,188,  10, 100, 100, 100, 100
    WAIT

    SPEED 10
    MOVE G6A, 60, 162,  30, 162, 145, 100
    MOVE G6D, 60, 162,  30, 162, 145, 100
    MOVE G6B,170,  10, 100, 100, 100, 100
    MOVE G6C,170,  10, 100, 100, 100, 100
    WAIT
    GOSUB Leg_motor_mode3	
    MOVE G6A, 60, 150,  28, 155, 140, 100
    MOVE G6D, 60, 150,  28, 155, 140, 100
    MOVE G6B,150,  60,  90, 100, 100, 100
    MOVE G6C,150,  60,  90, 100, 100, 100
    WAIT

    MOVE G6A,100, 150,  28, 140, 100, 100
    MOVE G6D,100, 150,  28, 140, 100, 100
    MOVE G6B,130,  50,  85, 100, 100, 100
    MOVE G6C,130,  50,  85, 100, 100, 100
    WAIT
    DELAY 100

    MOVE G6A,100, 150,  33, 140, 100, 100
    MOVE G6D,100, 150,  33, 140, 100, 100
    WAIT
    SPEED 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫

    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1

    DELAY 200
    GOSUB О©╫О©╫О©╫л╥О©╫ON

    RETURN


    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫о╬НЁ╙О©╫О©╫:


    HIGHSPEED SETOFF
    PTP SETON 				
    PTP ALLON

    GOSUB О©╫О©╫О©╫л╥О©╫OFF

    HIGHSPEED SETOFF

    GOSUB All_motor_Reset

    SPEED 15
    MOVE G6A,100, 15,  70, 140, 100,
    MOVE G6D,100, 15,  70, 140, 100,
    MOVE G6B,20,  140,  15
    MOVE G6C,20,  140,  15
    WAIT

    SPEED 12
    MOVE G6A,100, 136,  35, 80, 100,
    MOVE G6D,100, 136,  35, 80, 100,
    MOVE G6B,20,  30,  80
    MOVE G6C,20,  30,  80
    WAIT

    SPEED 12
    MOVE G6A,100, 165,  70, 30, 100,
    MOVE G6D,100, 165,  70, 30, 100,
    MOVE G6B,30,  20,  95
    MOVE G6C,30,  20,  95
    WAIT

    GOSUB Leg_motor_mode3

    SPEED 10
    MOVE G6A,100, 165,  45, 90, 100,
    MOVE G6D,100, 165,  45, 90, 100,
    MOVE G6B,130,  50,  60
    MOVE G6C,130,  50,  60
    WAIT

    SPEED 6
    MOVE G6A,100, 145,  45, 130, 100,
    MOVE G6D,100, 145,  45, 130, 100,
    WAIT


    SPEED 8
    GOSUB All_motor_mode2
    GOSUB О©╫Б╨╩О©╫з╪О©╫
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1

    '******************************
    DELAY 200
    GOSUB О©╫О©╫О©╫л╥О©╫ON
    RETURN

    '******************************************
    '******************************************
    '******************************************
    '**************************************************

    '******************************************
    '******************************************	
    '**********************************************

О©╫с╦О©╫О©╫О©╫О©╫О©╫30О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,70
    GOTO MAIN

О©╫с╦О©╫О©╫О©╫О©╫О©╫45О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,55
    GOTO MAIN

О©╫с╦О©╫О©╫О©╫О©╫О©╫60О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,40
    GOTO MAIN

О©╫с╦О©╫О©╫О©╫О©╫О©╫90О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,10
    GOTO MAIN

О©╫с╦О©╫О©╫О©╫О©╫О©╫О©╫О©╫30О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,130
    GOTO MAIN

О©╫с╦О©╫О©╫О©╫О©╫О©╫О©╫О©╫45О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,145
    GOTO MAIN	

О©╫с╦О©╫О©╫О©╫О©╫О©╫О©╫О©╫60О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,160
    GOTO MAIN

О©╫с╦О©╫О©╫О©╫О©╫О©╫О©╫О©╫90О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,190
    GOTO MAIN

О©╫с╦О©╫О©╫б©О©╫О©╫ъ╬О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,100
    GOTO MAIN

<<<<<<< HEAD
О©╫с╦О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    SPEED О©╫с╦О©╫О©╫л╣О©╫О©╫с╣О©╫
    SERVO 11,100	
=======
╦с╦╝╩Сгоа╓╦И:
    SPEED ╦с╦╝юл╣©╪с╣╣
    SERVO 11,100	' юл╟е ╦сюс? ©ж ╩Сгоюн╣╔ 11юл╬ъ
>>>>>>> cbc020c (Feat: М ÷Л┬≤_Л┐╓Л┌╜Л┐╓Л┌╜ Л╤■Й╟─)
    SPEED 5
    GOSUB О©╫Б╨╩О©╫з╪О©╫
    GOTO MAIN

    '******************************************

О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    FOR i = 0 TO COUNT_MAX
        A = AD(О©╫у╣з╠О©╫О©╫О©╫ADО©╫О©╫ф╝)	'О©╫О©╫О©╫О©╫ О©╫у╣О©╫
        IF A > 250 OR A < 5 THEN RETURN
        IF A > MIN AND A < MAX THEN RETURN
        DELAY О©╫О©╫О©╫О©╫х╝О©╫н╫ц╟О©╫
    NEXT i

    IF A < MIN THEN
        GOSUB О©╫О©╫О©╫О©╫О©╫
    ELSEIF A > MAX THEN
        GOSUB О©╫О©╫О©╫О©╫О©╫
    ENDIF

    RETURN
    '**************************************************
О©╫О©╫О©╫О©╫О©╫:
    A = AD(О©╫у╣з╠О©╫О©╫О©╫ADО©╫О©╫ф╝)
    'IF A < MIN THEN GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫о╬НЁ╙О©╫О©╫
    IF A < MIN THEN
        ETX  4800,16
        GOSUB О©╫з╥О©╫О©╫о╬НЁ╙О©╫О©╫

    ENDIF
    RETURN

О©╫О©╫О©╫О©╫О©╫:
    A = AD(О©╫у╣з╠О©╫О©╫О©╫ADО©╫О©╫ф╝)
    'IF A > MAX THEN GOSUB О©╫з╥О©╫О©╫о╬НЁ╙О©╫О©╫
    IF A > MAX THEN
        ETX  4800,15
        GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫о╬НЁ╙О©╫О©╫
    ENDIF
    RETURN
    '**************************************************
О©╫б©О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    FOR i = 0 TO COUNT_MAX
        B = AD(О©╫б©О©╫О©╫О©╫О©╫ADО©╫О©╫ф╝)	'О©╫О©╫О©╫О©╫ О©╫б©О©╫
        IF B > 250 OR B < 5 THEN RETURN
        IF B > MIN AND B < MAX THEN RETURN
        DELAY О©╫О©╫О©╫О©╫х╝О©╫н╫ц╟О©╫
    NEXT i

    IF B < MIN OR B > MAX THEN
        SPEED 8
        MOVE G6B,140,  40,  80
        MOVE G6C,140,  40,  80
        WAIT
        GOSUB О©╫Б╨╩О©╫з╪О©╫	
    ENDIF
    RETURN
    '******************************************
    '************************************************
SOUND_PLAY_CHK:
    DELAY 60
    SOUND_BUSY = IN(46)
    IF SOUND_BUSY = 1 THEN GOTO SOUND_PLAY_CHK
    DELAY 50

    RETURN
    '************************************************

    '************************************************
NUM_1_9:
    IF NUM = 1 THEN
        PRINT "1"
    ELSEIF NUM = 2 THEN
        PRINT "2"
    ELSEIF NUM = 3 THEN
        PRINT "3"
    ELSEIF NUM = 4 THEN
        PRINT "4"
    ELSEIF NUM = 5 THEN
        PRINT "5"
    ELSEIF NUM = 6 THEN
        PRINT "6"
    ELSEIF NUM = 7 THEN
        PRINT "7"
    ELSEIF NUM = 8 THEN
        PRINT "8"
    ELSEIF NUM = 9 THEN
        PRINT "9"
    ELSEIF NUM = 0 THEN
        PRINT "0"
    ENDIF

    RETURN
    '************************************************
    '************************************************
NUM_TO_ARR:

    NO_4 =  BUTTON_NO / 10000
    TEMP_INTEGER = BUTTON_NO MOD 10000

    NO_3 =  TEMP_INTEGER / 1000
    TEMP_INTEGER = BUTTON_NO MOD 1000

    NO_2 =  TEMP_INTEGER / 100
    TEMP_INTEGER = BUTTON_NO MOD 100

    NO_1 =  TEMP_INTEGER / 10
    TEMP_INTEGER = BUTTON_NO MOD 10

    NO_0 =  TEMP_INTEGER

    RETURN
    '************************************************
Number_Play: '  BUTTON_NO = О©╫О©╫О©╫з╢О©╫О©╫О©╫


    GOSUB NUM_TO_ARR

    PRINT "NPL "
    '*************

    NUM = NO_4
    GOSUB NUM_1_9

    '*************
    NUM = NO_3
    GOSUB NUM_1_9

    '*************
    NUM = NO_2
    GOSUB NUM_1_9
    '*************
    NUM = NO_1
    GOSUB NUM_1_9
    '*************
    NUM = NO_0
    GOSUB NUM_1_9
    PRINT " !"

    'GOSUB SOUND_PLAY_CHK
    PRINT "SND 16 !"
    'GOSUB SOUND_PLAY_CHK
    RETURN
    '************************************************

    RETURN


    '******************************************

    ' ************************************************
О©╫О©╫О©╫э╪О©╫О©╫е╦О©╫О©╫О©╫О©╫О©╫х╝О©╫О©╫:

    О©╫О©╫О©╫э╪О©╫О©╫е╦О©╫О©╫О©╫ = AD(О©╫О©╫О©╫э╪О©╫ADО©╫О©╫ф╝)

    IF О©╫О©╫О©╫э╪О©╫О©╫е╦О©╫О©╫О©╫ > 50 THEN '50 = О©╫О©╫О©╫э╪О©╫О©╫е╦О©╫О©╫О©╫ = 25cm
        'MUSIC "C"
        DELAY 200
    ENDIF




    RETURN

    '******************************************


    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫10:

    SPEED 5
    MOVE G6A,97,  86, 145,  75, 103, 100
    MOVE G6D,97,  66, 145,  95, 103, 100
    WAIT

    SPEED 12
    MOVE G6A,94,  86, 145,  75, 101, 100
    MOVE G6D,94,  66, 145,  95, 101, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  85, 98, 100
    MOVE G6D,101,  76, 146,  85, 98, 100
    WAIT

    MOVE G6A,100,  76, 145,  85, 100
    MOVE G6D,100,  76, 145,  85, 100
    WAIT
    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫10:

    SPEED 5
    MOVE G6A,97,  66, 145,  95, 103, 100
    MOVE G6D,97,  86, 145,  75, 103, 100
    WAIT

    SPEED 12
    MOVE G6A,94,  66, 145,  95, 101, 100
    MOVE G6D,94,  86, 145,  75, 101, 100
    WAIT
    SPEED 6
    MOVE G6A,101,  76, 146,  85, 98, 100
    MOVE G6D,101,  76, 146,  85, 98, 100
    WAIT

    MOVE G6A,100,  76, 145,  85, 100
    MOVE G6D,100,  76, 145,  85, 100
    WAIT
    GOTO RX_EXIT
    '**********************************************
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫20:

    GOSUB Leg_motor_mode2
    SPEED 8
    MOVE G6A,95,  96, 145,  65, 105, 100
    MOVE G6D,95,  56, 145,  105, 105, 100
    WAIT

    SPEED 12
    MOVE G6A,93,  96, 145,  65, 105, 100
    MOVE G6D,93,  56, 145,  105, 105, 100
    WAIT
    SPEED 6
    MOVE G6A,101,  76, 146,  85, 98, 100
    MOVE G6D,101,  76, 146,  85, 98, 100
    WAIT

    MOVE G6A,100,  76, 145,  85, 100
    MOVE G6D,100,  76, 145,  85, 100
    WAIT
    GOSUB Leg_motor_mode1
    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫20:

    GOSUB Leg_motor_mode2
    SPEED 8
    MOVE G6A,95,  56, 145,  105, 105, 100
    MOVE G6D,95,  96, 145,  65, 105, 100
    WAIT

    SPEED 12
    MOVE G6A,93,  56, 145,  105, 105, 100
    MOVE G6D,93,  96, 145,  65, 105, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  85, 98, 100
    MOVE G6D,101,  76, 146,  85, 98, 100
    WAIT

    MOVE G6A,100,  76, 145,  85, 100
    MOVE G6D,100,  76, 145,  85, 100
    WAIT

    GOSUB Leg_motor_mode1
    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45:

    GOSUB Leg_motor_mode2
    SPEED 8
    MOVE G6A,95,  106, 145,  59, 105, 100
    MOVE G6D,95,  46, 145,  119, 105, 100
    WAIT

    SPEED 10
    MOVE G6A,93,  106, 145,  59, 105, 100
    MOVE G6D,93,  46, 145,  119, 105, 100
    WAIT

    SPEED 8
    MOVE G6A,100,  76, 145,  89, 100
    MOVE G6D,100,  76, 145,  89, 100
    WAIT
    GOSUB Leg_motor_mode1
    GOTO RX_EXIT

    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45:

    GOSUB Leg_motor_mode2
    SPEED 8
    MOVE G6A,95,  46, 145,  119, 105, 100
    MOVE G6D,95,  106, 145,  59, 105, 100
    WAIT

    SPEED 10
    MOVE G6A,93,  46, 145,  119, 105, 100
    MOVE G6D,93,  106, 145,  59, 105, 100
    WAIT

    SPEED 8
    MOVE G6A,100,  76, 145,  89, 100
    MOVE G6D,100,  76, 145,  89, 100
    WAIT
    GOSUB Leg_motor_mode1
    GOTO RX_EXIT
    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫60:

    SPEED 15
    MOVE G6A,95,  116, 145,  45, 105, 100
    MOVE G6D,95,  36, 145,  125, 105, 100
    WAIT

    SPEED 15
    MOVE G6A,90,  116, 145,  45, 105, 100
    MOVE G6D,90,  36, 145,  125, 105, 100
    WAIT

    SPEED 10
    MOVE G6A,100,  76, 145,  85, 100
    MOVE G6D,100,  76, 145,  85, 100
    WAIT
    GOTO RX_EXIT

    '**********************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫60:

    SPEED 15
    MOVE G6A,95,  36, 145,  125, 105, 100
    MOVE G6D,95,  116, 145,  45, 105, 100
    WAIT

    SPEED 15
    MOVE G6A,90,  36, 145,  125, 105, 100
    MOVE G6D,90,  116, 145,  45, 105, 100
    WAIT

    SPEED 10
    MOVE G6A,100,  76, 145,  85, 100
    MOVE G6D,100,  76, 145,  85, 100
    WAIT
    GOTO RX_EXIT

    '************************************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP:
    MOVE G6B, 143, 10, 60,	  ,	  ,
    MOVE G6C, 143, 10, 60,	  ,   ,
    WAIT
    DELAY 3
    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 12
        MOVE G6A,102,  73, 145,  93, 100, 100
        MOVE G6D,102,  79, 145,  93, 100, 100
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,100,  79, 145,  93, 100, 100
        WAIT


        SPEED 5
        MOVE G6A,102,  84, 145,  78, 100, 100
        MOVE G6D,102,  68, 145,  108, 100, 100
        MOVE G6A,100,  84, 145,  78, 100, 100
        MOVE G6D,100,  68, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6A,90,  90, 145,  78, 102, 100
        MOVE G6D,104,  71, 145,  105, 100, 100
        WAIT
        SPEED 6
        MOVE G6A,90,  80, 130, 102, 104
        MOVE G6D,105,  76, 146,  93,  100
        WAIT
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 12
        MOVE G6A,102,  73, 145,  93, 100, 100
        MOVE G6D,102,  79, 145,  93, 100, 100
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,100,  79, 145,  93, 100, 100
        WAIT


        SPEED 5
        MOVE G6A,102,  88, 145,  78, 100, 100
        MOVE G6D,102,  65, 145,  108, 100, 100
        MOVE G6A,100,  88, 145,  78, 100, 100
        MOVE G6D,100,  65, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6A,104,  86, 146,  80, 100, 100
        MOVE G6D,90,  58, 145,  110, 100, 100
        WAIT
        SPEED 6
        MOVE G6D,90,  85, 130, 98, 104
        MOVE G6A,105,  77, 146,  93,  100
        WAIT
    ENDIF
    SPEED 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    GOTO RX_EXIT
    '*************************************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3_LOOP:
    MOVE G6B, 143, 10, 60,	  ,	  ,
    MOVE G6C, 143, 10, 60,	  ,   ,
    WAIT
    DELAY 3

    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 12
        MOVE G6D,102,  73, 145,  93, 100, 100
        MOVE G6A,102,  79, 145,  93, 100, 100
        WAIT

        SPEED 5
        MOVE G6D,102,  84, 145,  78, 100, 100
        MOVE G6A,102,  68, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6D,90,  90, 145,  78, 102, 100
        MOVE G6A,104,  71, 145,  105, 100, 100
        WAIT
        SPEED 6
        MOVE G6D,90,  80, 130, 102, 104
        MOVE G6A,105,  76, 146,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 12
        MOVE G6D,102,  73, 145,  93, 100, 100
        MOVE G6A,102,  79, 145,  93, 100, 100
        WAIT


        SPEED 5
        MOVE G6D,102,  88, 145,  78, 100, 100
        MOVE G6A,102,  65, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6D,104,  86, 146,  80, 100, 100
        MOVE G6A,90,  58, 145,  110, 100, 100
        WAIT

        SPEED 6
        MOVE G6A,90,  85, 130, 98, 104
        MOVE G6D,105,  77, 146,  93,  100
        WAIT



    ENDIF

    SPEED 10
    '    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT

    GOTO RX_EXIT


    '************************************************
    '************************************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    PRINT "OPEN 20GongMo.mrs !"
    PRINT "SOUND 4 !"
    GOSUB SOUND_PLAY_CHK
    RETURN

х╝О©╫О©╫О©╫О©╫О©╫О©╫:
    PRINT "OPEN 20GongMo.mrs !"
    PRINT "SOUND 5 !"
    GOSUB SOUND_PLAY_CHK
    RETURN

О©╫О©╫О©╫О©╫:
    SPEED 10
    MOVE G6C, 190, 30, 80
    WAIT
    PRINT "OPEN 20GongMo.mrs !"
    PRINT "SOUND 0 !"
    GOSUB SOUND_PLAY_CHK
    DELAY 500
    PRINT "SOUND 0 !"
    DELAY 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN

О©╫О©╫О©╫О©╫:
    SPEED 10
    MOVE G6B, 190, 30, 80
    WAIT
    PRINT "OPEN 20GongMo.mrs !"
    PRINT "SOUND 1 !"
    GOSUB SOUND_PLAY_CHK
    DELAY 500
    PRINT "SOUND 1 !"
    DELAY 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫2	
    RETURN
О©╫О©╫О©╫О©╫:
    SPEED 10
    MOVE G6B, 30, 30, 80
    MOVE G6C, 30, 30, 80
    WAIT
    PRINT "OPEN 20GongMo.mrs !"
    PRINT "SOUND 2 !"
    GOSUB SOUND_PLAY_CHK
    DELAY 500
    PRINT "SOUND 2 !"
    DELAY 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN
О©╫О©╫О©╫О©╫:
    SPEED 10
    MOVE G6B, 190, 30, 80
    MOVE G6C, 190, 30, 80
    WAIT
    PRINT "OPEN 20GongMo.mrs !"
    PRINT "SOUND 3 !"
    DELAY 500
    PRINT "SOUND 3 !"
    DELAY 10
<<<<<<< HEAD
    'GOSUB SOUND_PLAY_CHK
    GOSUB О©╫Б╨╩О©╫з╪О©╫2
=======
    GOSUB SOUND_PLAY_CHK
    GOSUB ╠Б╨╩юз╪╪2
>>>>>>> 2bc48b6 (Chore: Л²▄Л└╠ Й╢─К═╗ К╙╗Л┘≤ Л┬≤Л═∙)
    RETURN
    '******************************************

AО©╫О©╫О©╫О©╫:
    PRINT "OPEN M_ABCD.mrs !"
    PRINT "SOUND 0 !"
    RETURN
BО©╫О©╫О©╫О©╫:
    PRINT "OPEN M_ABCD.mrs !"
    PRINT "SOUND 1 !"
    RETURN
CО©╫О©╫О©╫О©╫:
    PRINT "OPEN M_ABCD.mrs !"
    PRINT "SOUND 2 !"
    RETURN
DО©╫О©╫О©╫О©╫:
    PRINT "OPEN M_ABCD.mrs !"
    PRINT "SOUND 3 !"
    RETURN

<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫100О©╫О©╫:
=======
юЭ╧ФгогБ110╣╣:
    SPEED 3
    SERVO 16, 110

    RETURN
    '******************************************
юЭ╧ФгогБ105╣╣:
    SPEED 3
    SERVO 16, 105

    RETURN
    '******************************************
юЭ╧ФгогБ100╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)
    SPEED 3
    SERVO 16, 100

    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫97О©╫О©╫:
    SPEED 3
    SERVO 16, 97

    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫95О©╫О©╫:
    SPEED 3
    SERVO 16, 95

    RETURN
    '******************************************
<<<<<<< HEAD

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫90О©╫О©╫:
=======
юЭ╧ФгогБ90╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)

    SPEED 3
    SERVO 16, 92

    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫85О©╫О©╫:

    SPEED 3
    SERVO 16, 85

    RETURN
    '******************************************
<<<<<<< HEAD

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫80О©╫О©╫:
=======
юЭ╧ФгогБ80╣╣:
>>>>>>> cbc020c (Feat: М ÷Л┬≤_Л┐╓Л┌╜Л┐╓Л┌╜ Л╤■Й╟─)

    SPEED 3
    SERVO 16, 80

    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫75О©╫О©╫:
    SPEED 3
    SERVO 16, 76

    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫70О©╫О©╫:
    SPEED 3
    SERVO 16, 73

    RETURN
    '******************************************
<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫60О©╫О©╫:
=======
юЭ╧ФгогБ65╣╣:
    SPEED 3
    SERVO 16, 69

    RETURN
    '******************************************
юЭ╧ФгогБ60╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)

    SPEED 3
    SERVO 16, 65

    RETURN

    '******************************************
<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫54О©╫О©╫:
=======
юЭ╧ФгогБ55╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)

    SPEED 3
    SERVO 16, 59

    RETURN

    '******************************************
<<<<<<< HEAD

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫50О©╫О©╫:

=======
юЭ╧ФгогБ50╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)
    SPEED 3
    SERVO 16, 54

    RETURN
    '******************************************

<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45О©╫О©╫:

=======
юЭ╧ФгогБ45╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)
    SPEED 3
    SERVO 16, 50
    
    RETURN
    '******************************************
юЭ╧ФгогБ40╣╣:
    SPEED 3
    SERVO 16, 45

    RETURN
    '******************************************
<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫35О©╫О©╫:

=======
юЭ╧ФгогБ35╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)
    SPEED 3
    SERVO 16, 40
    RETURN

    '******************************************
<<<<<<< HEAD

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫30О©╫О©╫:
=======
юЭ╧ФгогБ30╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)

    SPEED 3
    SERVO 16, 36

    RETURN
    '******************************************
юЭ╧ФгогБ25╣╣:
    SPEED 3
    SERVO 16, 30

    RETURN
    '******************************************
юЭ╧ФгогБ20╣╣:
    SPEED 3
    SERVO 16, 26

    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫18О©╫О©╫:

    SPEED 3
    SERVO 16, 22
    
    RETURN
    '******************************************
<<<<<<< HEAD

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫10О©╫О©╫:
=======
юЭ╧ФгогБ10╣╣:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)

    SPEED 3
    SERVO 16, 10

    RETURN
    '******************************************

<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫110О©╫О©╫:

    SPEED 3
    SERVO 16, 110
    '   ETX 4800,41

    RETURN

О©╫О©╫О©╫х╧О©╫О©╫О©╫О©╫О©╫:
=======
╬Гфх╧З╦╝╠Б:
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)
    'MOVE G6A, 101,  83, 128,  96,  99, 100
    'MOVE G6D, 100,  79, 128, 100,  99, 100
    MOVE G6B, 107, 101, 100, 100, 100, 101
    MOVE G6C, 107, 101, 100, 100, 100, 100
    WAIT


    RETURN

О©╫О©╫О©╫Б╨╩О©╫з╪О©╫:
    MOVE G6B, 90, 80, 80, 90, 100, 101
    MOVE G6C, 90, 80, 80, 90, 100, 100
    WAIT

    RETURN


О©╫О©╫О©╫х╬О©╫О©╫О©╫О©╫О©╫:
    SPEED 4
    MOVE G6B, 185, 10, 80
    MOVE G6C, 190, 10, 80
    WAIT

    'DELAY 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN

О©╫О©╫О©╫х╬О©╫О©╫О©╫О©╫н©О©╫О©╫й╟х╠О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    MOVE G6B, 185, 10, 80
    MOVE G6C, 190, 10, 80
    WAIT

    DELAY 3

    SPEED 12
    MOVE G6A, 95,  90, 125, 100, 104, 100
    MOVE G6D,105,  76, 145,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6A, 102,  77, 145, 93, 100, 100
    MOVE G6D,90,  80, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6A,95,  76, 145,  93, 102, 100
    MOVE G6D,95,  76, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100


    GOSUB All_motor_mode3
    GOTO RX_EXIT


    '******************************************	
О©╫О©╫О©╫х╬О©╫О©╫О©╫О©╫н©О©╫О©╫О©╫О©╫й╟х╠О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    MOVE G6B, 185, 10, 80
    MOVE G6C, 190, 10, 80
    WAIT

    DELAY 3

    SPEED 12
    MOVE G6D, 95,  90, 125, 100, 104, 100
    MOVE G6A,105,  76, 146,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  77, 145, 93, 100, 100
    MOVE G6A,90,  80, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6D,95,  76, 145,  93, 102, 100
    MOVE G6A,95,  76, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOSUB All_motor_mode3
    GOTO RX_EXIT




О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    GOSUB О©╫О©╫О©╫л╥О©╫OFF
    SPEED 8  'before 2
    MOVE G6A, 100, 150, 30,   150, 100,	
    MOVE G6D, 100, 150, 30,   150, 100,
    MOVE G6B, 140, 30, 80,	  ,	  ,
    MOVE G6C, 140, 30, 80,	  ,   ,
    WAIT

    DELAY 50
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:
    ETX 4800, 46
    SPEED 4  'before 4
    'DELAY 20
    MOVE G6A, 100, 150, 30,   150, 100,	
    MOVE G6D, 100, 150, 30,   150, 100,
    MOVE G6B, 150, 10, 60,	  ,	  ,
    MOVE G6C, 150, 10, 60,	  ,   ,
    WAIT

    DELAY 20
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    ETX 4800, 46
    SPEED 8   'before 4
    MOVE G6A,100, 76,  145,    93,  100, 100
    MOVE G6D,100, 76,  145,    93,  100, 100
    MOVE G6B, 150, 10, 60,	  ,	  ,
    MOVE G6C, 150, 10, 60,	  ,   ,
    WAIT

    'DELAY 20
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    ETX 4800, 46
    SPEED 8  'before 4
    MOVE G6A,100, 76,  145,    93,  100, 100
    MOVE G6D,100, 76,  145,    93,  100, 100
    MOVE G6B, 160
    MOVE G6C, 160
    WAIT

    RETURN
    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    MOVE G6B, 175, 10, 60,	  ,	  ,
    MOVE G6C, 175, 10, 60,	  ,   ,
    WAIT

    DELAY 3

    SPEED 12
    MOVE G6A, 95,  90, 125, 100, 104, 100
    MOVE G6D,105,  76, 145,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6A, 102,  77, 145, 93, 100, 100
    MOVE G6D,90,  80, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6A,95,  76, 145,  93, 102, 100
    MOVE G6D,95,  76, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100


    GOSUB All_motor_mode3
    GOTO RX_EXIT


    '******************************************	
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    MOVE G6B, 175, 10, 60,	  ,	  ,
    MOVE G6C, 175, 10, 60,	  ,   ,
    WAIT

    DELAY 3

    SPEED 12
    MOVE G6D, 95,  90, 125, 100, 104, 100
    MOVE G6A,105,  76, 146,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  77, 145, 93, 100, 100
    MOVE G6A,90,  80, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6D,95,  76, 145,  93, 102, 100
    MOVE G6A,95,  76, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOSUB All_motor_mode3
    GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫2:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫2_LOOP:
    DELAY  10

    SPEED 10
    MOVE G6D, 90,  90, 120, 105, 110, 100
    MOVE G6A,100,  76, 145,  93, 107, 100
    MOVE G6B, 175, 10, 60,	  ,	  ,
    MOVE G6C, 175, 10, 60,	  ,   ,
    WAIT

    SPEED 13
    MOVE G6D, 102,  76, 145, 93, 100, 100
    MOVE G6A,83,  78, 140,  96, 115, 100
    WAIT

    SPEED 13
    MOVE G6D,98,  76, 145,  93, 100, 100
    MOVE G6A,98,  76, 145,  93, 100, 100
    WAIT

    SPEED 12
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT


    '  ERX 4800, A ,О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop
    '    IF A = A_OLD THEN  GOTO О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop
    'О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_stop:
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOTO RX_EXIT
    '**********************************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫2:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫2_LOOP:
    DELAY  10

    SPEED 10
    MOVE G6A, 90,  90, 120, 105, 110, 100	
    MOVE G6D,100,  76, 145,  93, 107, 100	
    MOVE G6B, 175, 10, 60,	  ,	  ,
    MOVE G6C, 175, 10, 60,	  ,   ,
    WAIT

    SPEED 13
    MOVE G6A, 102,  76, 145, 93, 100, 100
    MOVE G6D,83,  78, 140,  96, 115, 100
    WAIT

    SPEED 13
    MOVE G6A,98,  76, 145,  93, 100, 100
    MOVE G6D,98,  76, 145,  93, 100, 100
    WAIT

    SPEED 12
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    WAIT

    '   ERX 4800, A ,О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop	
    '    IF A = A_OLD THEN  GOTO О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_loop
    'О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫70О©╫О©╫О©╫О©╫_stop:

    'GOSUB О©╫Б╨╩О©╫з╪О©╫
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOTO RX_EXIT

    '******************************************

    '******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    '   MOVE G6B, 175, 10, 60,	  ,	  ,
    '  MOVE G6C, 175, 10, 60,	  ,   ,
    WAIT

    DELAY 3

    SPEED 12
    MOVE G6A, 95,  90, 125, 100, 104, 100
    MOVE G6D,105,  76, 145,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6A, 102,  77, 145, 93, 100, 100
    MOVE G6D,90,  80, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6A,95,  76, 145,  93, 102, 100
    MOVE G6D,95,  76, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100


    GOSUB All_motor_mode3
    GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    '   MOVE G6B, 175, 10, 60,	  ,	  ,
    '  MOVE G6C, 175, 10, 60,	  ,   ,
    '   MOVE G6B, 175, 10, 60,	  ,	  ,
    '  MOVE G6C, 175, 10, 60,	  ,   ,
    WAIT

    DELAY 3

    SPEED 12
    MOVE G6D, 95,  90, 125, 100, 104, 100
    MOVE G6A,105,  76, 146,  93, 104, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  77, 145, 93, 100, 100
    MOVE G6A,90,  80, 140,  95, 107, 100
    WAIT

    SPEED 10
    MOVE G6D,95,  76, 145,  93, 102, 100
    MOVE G6A,95,  76, 145,  93, 102, 100
    WAIT

    SPEED 8
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100

    GOSUB All_motor_mode3
    GOTO RX_EXIT


О©╫О©╫О©╫гЁО©╫О©╫О©╫:
    GOSUB All_motor_mode3
    GOSUB О©╫О©╫О©╫л╥О©╫OFF
    MOVE G6B, 145, , ,
    MOVE G6C, 145, , ,
    WAIT

    MOVE G6B, 140, 10, 60,	  ,	  ,
    MOVE G6C, 140, 10, 60,	  ,   ,
    WAIT

    GOTO О©╫О©╫О©╫гЁО©╫О©╫О©╫_2	
О©╫О©╫О©╫гЁО©╫О©╫О©╫_2:
    ETX 4800, 47
    MOVE G6A, 100, 150, 30,   150, 100,	
    MOVE G6D, 100, 150, 30,   150, 100,
    MOVE G6B, 150, , ,
    MOVE G6C, 150, , ,
    WAIT

    GOTO О©╫О©╫О©╫гЁО©╫О©╫О©╫_3
О©╫О©╫О©╫гЁО©╫О©╫О©╫_3:
    ETX 4800, 47
    MOVE G6B, , 30, 90
    MOVE G6C, , 30, 90
    WAIT

    GOTO О©╫О©╫О©╫гЁО©╫О©╫О©╫_4
О©╫О©╫О©╫гЁО©╫О©╫О©╫_4:
    ETX 4800, 47
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT

    GOSUB О©╫Б╨╩О©╫з╪О©╫
    RETURN

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    'HIGHSPEED SETON
    MOVE G6B, 160, 10, 50
    MOVE G6C, 160, 10, 50


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 147,  93, 101
        MOVE G6D,101,  76, 147,  93, 98
        'MOVE G6B,100
        'MOVE G6C,100
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 147,  93, 101
        MOVE G6A,101,  76, 147,  93, 98
        'MOVE G6B,100
        'MOVE G6C,100
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF


    '**********************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    MOVE G6A,95,  90, 125, 100, 104
    MOVE G6D,104,  77, 147,  93,  102
    'MOVE G6B, 85
    'MOVE G6C,115
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:

    MOVE G6A,103,   73, 140, 103,  100
    MOVE G6D, 95,  85, 147,  85, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

        GOTO RX_EXIT
    ENDIF

    ' О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    'IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2_stop:
        MOVE G6D,95,  90, 125, 95, 104
        MOVE G6A,104,  76, 145,  91,  102
        'MOVE G6C, 100
        'MOVE G6B,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*********************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    MOVE G6D,95,  95, 120, 100, 104
    MOVE G6A,104,  77, 147,  93,  102
    'MOVE G6C, 85
    'MOVE G6B,115
    WAIT


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5:
    MOVE G6D,103,    73, 140, 103,  100
    MOVE G6A, 95,  85, 147,  85, 102
    WAIT


    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    ' О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    ' IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_5_stop:
        MOVE G6A,95,  90, 125, 95, 104
        MOVE G6D,104,  76, 145,  91,  102
        'MOVE G6B, 100
        'MOVE G6C,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 5
        GOSUB О©╫Б╨╩О©╫з╪О©╫2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    О©╫О©╫О©╫О©╫с╣О©╫ = 8
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    'GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫18О©╫О©╫
    'DELAY 20
    SPEED 6
    GOSUB All_motor_mode3
    MOVE G6B, 160, 10, 50
    MOVE G6C, 160, 10, 50
    WAIT

    DELAY 20
    'HIGHSPEED SETON

    SPEED 6
    MOVE G6D,  90,  74, 144,  95, 110
    MOVE G6A, 108,  76, 146,  93, 96
    WAIT

    SPEED 8
    MOVE G6D,90, 90, 120, 102, 110,100
    MOVE G6A,108,  76, 147,  93,  96,100
    WAIT

    'HIGHSPEED SETOFF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2	

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:
    MOVE G6A,110,  76, 147,  93, 100,100
    MOVE G6D,96, 90, 120, 102, 107,100
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    'ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 90,  56, 145, 115, 110
    MOVE G6A,108,  76, 147,  90,  96
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,108,  76, 147, 90,  98
    MOVE G6A,90, 100, 142,  69, 108
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    SPEED 9
    MOVE G6A,95, 90, 120, 102, 111,100
    MOVE G6D,108,  76, 146,  93,  96,100
    WAIT

    SPEED 7
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT

    RETURN

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    О©╫О©╫О©╫О©╫с╣О©╫ = 8
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    'GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫18О©╫О©╫
    'DELAY 20
    SPEED 10
    GOSUB All_motor_mode3
    MOVE G6B,185,  10,  60
    MOVE G6C,185,  10,  60
    WAIT

    DELAY 20
    'HIGHSPEED SETON

    SPEED 10
    MOVE G6D,  90,  74, 142,  94, 109
    MOVE G6A, 108,  76, 144,  93, 96
    WAIT

    SPEED 12
    MOVE G6D,90, 90, 118, 101, 109,99
    MOVE G6A,108,  76, 145,  92,  96,100
    WAIT

    'HIGHSPEED SETOFF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2	

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:
    MOVE G6A,110,  76, 145,  93, 100,100
    MOVE G6D,96, 90, 118, 101, 106,99
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    'ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 90,  56, 143, 114, 109
    MOVE G6A,108,  76, 145,  89,  95
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,108,  76, 145, 89,  97
    MOVE G6A,90, 100, 140,  68, 107
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    SPEED 13
    MOVE G6A,95, 90, 118, 101, 110,99
    MOVE G6D,108,  76, 144,  92,  95,99
    WAIT

    SPEED 11
    MOVE G6A,100,  76, 143,  92, 99, 100
    MOVE G6D,100,  76, 143,  92, 99, 100
    WAIT

    GOTO RX_EXIT
    '*******************
<<<<<<< HEAD
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
    О©╫О©╫О©╫О©╫с╣О©╫ = 8
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    'GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫18О©╫О©╫
    'DELAY 20
    SPEED 6
    GOSUB All_motor_mode3
    MOVE G6B, 185, 10, 80
    MOVE G6C, 190, 10, 80
    WAIT

    DELAY 20
    'HIGHSPEED SETON

    SPEED 10
    MOVE G6D,  90,  74, 144,  94, 109
    MOVE G6A, 108,  76, 146,  93, 96
    WAIT

    SPEED 12
    MOVE G6D,90, 90, 120, 101, 109,99
    MOVE G6A,108,  76, 147,  92,  96,100
    WAIT

    'HIGHSPEED SETOFF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2	

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:
    MOVE G6A,110,  76, 147,  93, 100,100
    MOVE G6D,96, 90, 120, 101, 106,99
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    'ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 90,  56, 145, 114, 109
    MOVE G6A,108,  76, 147,  89,  95
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,108,  76, 147, 89,  97
    MOVE G6A,90, 100, 142,  68, 107
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    SPEED 13
    MOVE G6A,95, 90, 120, 101, 110,99
    MOVE G6D,108,  76, 146,  92,  95,99
    WAIT

    SPEED 11
    MOVE G6A,100,  76, 145,  92, 99, 100
    MOVE G6D,100,  76, 145,  92, 99, 100
    WAIT

    RETURN
х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫:
=======
х╫╪Ж_аЩ╟МхдаЬ:
>>>>>>> 090d4a9 (Chore: К╛╦Л≈╢ЙЁ═ К▀╛К╕╛Й╦╟ К╙╗Л┘≤ Л┌╜Л═°)
    GOSUB All_motor_mode3
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    О©╫О©╫О©╫О©╫COUNT = 0
    SPEED 7
    'HIGHSPEED SETON


    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 145,  93, 101
        MOVE G6D,101,  76, 145,  93, 98
        MOVE G6B, 190, 10, 50
        MOVE G6C, 190, 10, 50
        WAIT

        GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 145,  93, 101
        MOVE G6A,101,  76, 145,  93, 98
        MOVE G6B, 190, 10, 50
        MOVE G6C, 190, 10, 50
        WAIT

        GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    ENDIF

<<<<<<< HEAD

<<<<<<< HEAD
    '**********************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50:
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    DELAY 50
    SPEED 6
    HIGHSPEED SETON

    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 145,  93, 101
        MOVE G6D,101,  77, 146,  93, 98
        WAIT

        MOVE G6A,95,  80, 120, 120, 104
        MOVE G6D,104,  77, 147,  91,  102
        MOVE G6B, 185, 10, 80
        MOVE G6C, 190, 10, 80
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_2
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 146,  93, 101
        MOVE G6A,101,  77, 145,  93, 98
        WAIT

        MOVE G6D,95,  80, 121, 120, 104
        MOVE G6A,104,  77, 146,  91,  102
        MOVE G6B, 185, 10, 80
        MOVE G6C, 190, 10, 80
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_5
    ENDIF
    '**********************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1:
    MOVE G6A,95,  95, 100, 120, 104
    MOVE G6D,104,  77, 148,  93,  102
    MOVE G6B, 185, 10, 80
    MOVE G6C, 190, 10, 80
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_2:
    MOVE G6A,95,  75, 122, 120, 104
    MOVE G6D,104,  78, 148,  90,  100
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3:
    MOVE G6A,103,  69, 145, 103,  100
    MOVE G6D, 95, 87, 161,  68, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3_stop:

        MOVE G6D,90,  93, 116, 100, 104
        MOVE G6A,104,  74, 145,  91,  102
        MOVE G6B, 185, 10, 80
        MOVE G6C, 190, 10, 80
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        ' GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 13
        ' GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  76, 145,  91, 100, 100
        MOVE G6D,100,  76, 145,  91, 100, 100

        DELAY 150
        GOTO RX_EXIT
    ENDIF
    '*********************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_4:
    MOVE G6D,95,  95, 101, 120, 104
    MOVE G6A,104,  77, 147,  93,  102
    MOVE G6B, 185, 10, 80
    MOVE G6C, 190, 10, 80
    WAIT
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_5:
    MOVE G6D,95,  75, 123, 120, 104
    MOVE G6A,104,  78, 147,  90,  100
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6:
    MOVE G6D,103,  69, 146, 103,  100
    MOVE G6A, 95, 87, 160,  68, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF
    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6_stop
    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6_stop:

        MOVE G6A,90,  93, 115, 100, 104
        MOVE G6D,104,  74, 146,  91,  102
        MOVE G6B, 185, 10, 80
        MOVE G6C, 190, 10, 80
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        ' GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        SPEED 13
        ' GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  76, 145,  91, 100, 100
        MOVE G6D,100,  76, 145,  91, 100, 100
        DELAY 150
        GOTO RX_EXIT
    ENDIF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1





=======
>>>>>>> 090d4a9 (Chore: К╛╦Л≈╢ЙЁ═ К▀╛К╕╛Й╦╟ К╙╗Л┘≤ Л┌╜Л═°)
    '*******************************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50:
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
    GOSUB All_motor_mode3
    О©╫О©╫О©╫О©╫COUNT = 0
    DELAY 50
    SPEED 6
    HIGHSPEED SETON

    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        MOVE G6A,95,  76, 145,  93, 101
        MOVE G6D,101,  77, 146,  93, 98
        WAIT

        MOVE G6A,95,  80, 120, 120, 104
        MOVE G6D,104,  77, 147,  91,  102
        MOVE G6B,185,  10,  60
        MOVE G6C,185,  10,  60
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_2
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        MOVE G6D,95,  76, 146,  93, 101
        MOVE G6A,101,  77, 145,  93, 98
        WAIT

        MOVE G6D,95,  80, 121, 120, 104
        MOVE G6A,104,  77, 146,  91,  102
        MOVE G6B,185,  10,  60
        MOVE G6C,185,  10,  60
        WAIT

        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_5
    ENDIF
    '**********************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1:
    MOVE G6A,95,  95, 100, 120, 104
    MOVE G6D,104,  77, 148,  93,  102
    MOVE G6B,185,  10,  60
    MOVE G6C,185,  10,  60
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_2:
    MOVE G6A,95,  75, 122, 120, 104
    MOVE G6D,104,  78, 148,  90,  100
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3:
    MOVE G6A,103,  69, 145, 103,  100
    MOVE G6D, 95, 87, 161,  68, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3_stop

    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_4
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_3_stop:

        MOVE G6D,90,  93, 116, 100, 104
        MOVE G6A,104,  74, 145,  91,  102
        MOVE G6B,185,  10,  60
        MOVE G6C,185,  10,  60
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        ' GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        ' GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  76, 145,  91, 100, 100
        MOVE G6D,100,  76, 145,  91, 100, 100

        DELAY 150
        GOTO RX_EXIT
    ENDIF
    '*********************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_4:
    MOVE G6D,95,  95, 101, 120, 104
    MOVE G6A,104,  77, 147,  93,  102
    MOVE G6B,185,  10,  60
    MOVE G6C,185,  10,  60
    WAIT
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_5:
    MOVE G6D,95,  75, 123, 120, 104
    MOVE G6A,104,  78, 147,  90,  100
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6:
    MOVE G6D,103,  69, 146, 103,  100
    MOVE G6A, 95, 87, 160,  68, 102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF
    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6_stop
    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1
    IF A <> A_old THEN
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_6_stop:

        MOVE G6A,90,  93, 115, 100, 104
        MOVE G6D,104,  74, 146,  91,  102
        MOVE G6B,185,  10,  60
        MOVE G6C,185,  10,  60
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        ' GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        ' GOSUB О©╫Б╨╩О©╫з╪О©╫2
        MOVE G6A,100,  76, 145,  91, 100, 100
        MOVE G6D,100,  76, 145,  91, 100, 100
        DELAY 150
        GOTO RX_EXIT
    ENDIF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫ч╦О©╫О©╫О©╫50_1


х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
=======
х╫╪Ж_аЩ╟МхдаЬ_1:
>>>>>>> a915980 (Chore: Л╖▒ЙЁ═Л═└Л╖└К▀╛К▀╛К╕╛Й╦╟ К╙╗Л┘≤ Л┌╜Л═°)
    MOVE G6D,104,  76, 147,  93,  102
    MOVE G6A,95,  95, 120, 95, 104
    MOVE G6B, 190, 10, 50
    MOVE G6C, 190, 10, 50
    WAIT



х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    MOVE G6A, 103,  79, 147,  89, 100
    MOVE G6D,95,   65, 147, 103,  102
    WAIT

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF
    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3_stop
    ERX 4800,A, х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4
    IF A <> A_old THEN
х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3_stop:
        MOVE G6D,95,  85, 130, 100, 104
        MOVE G6A,104,  77, 146,  93,  102
        MOVE G6B, 190, 10, 50
        MOVE G6C, 190, 10, 50
        WAIT

        'SPEED 15
        '        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        '       HIGHSPEED SETOFF
        '      SPEED 5
        '     GOSUB О©╫Б╨╩О©╫з╪О©╫2

        '   DELAY 400
        GOTO RX_EXIT
    ENDIF
    '*********************************

х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_4:
    MOVE G6A,104,  76, 147,  93,  102
    MOVE G6D,95,  95, 120, 95, 104
    MOVE G6B, 190, 10, 50
    MOVE G6C, 190, 10, 50
    WAIT


х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6:
    MOVE G6D, 103,  79, 147,  89, 100
    MOVE G6A,95,   65, 147, 103,  102
    WAIT
    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO RX_EXIT
    ENDIF

    О©╫О©╫О©╫О©╫COUNT = О©╫О©╫О©╫О©╫COUNT + 1
    IF О©╫О©╫О©╫О©╫COUNT > О©╫О©╫О©╫О©╫х╫О©╫О©╫ THEN  GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6_stop

    ERX 4800,A, х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1
    IF A <> A_old THEN  'GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_О©╫О©╫О©╫О©╫
х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_6_stop:
        MOVE G6A,95,  85, 130, 100, 104
        MOVE G6D,104,  77, 146,  93,  102
        MOVE G6B, 190, 10, 50
        MOVE G6C, 190, 10, 50
        WAIT

        'SPEED 15
        '        GOSUB О©╫О©╫О©╫О©╫х╜О©╫з╪О©╫
        '       HIGHSPEED SETOFF
        '      SPEED 5
        '     GOSUB О©╫Б╨╩О©╫з╪О©╫2

        '  DELAY 400
        GOTO RX_EXIT
    ENDIF

    GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    SPEED 4
    MOVE G6B, 190, 20, 60
    MOVE G6C, 190, 20, 60
    WAIT

    'DELAY 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:
    SPEED 4
    MOVE G6B, 150, 20, 60
    MOVE G6C, 150, 20, 60
    WAIT

    'DELAY 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_1:
    SPEED 4
    MOVE G6B, 190, 10, 50
    MOVE G6C, 190, 10, 50
    WAIT

    'DELAY 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN
    'GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_2:
    SPEED 4
    MOVE G6B, 150, 10, 50
    MOVE G6C, 150, 10, 50
    WAIT

    'DELAY 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN
    ' GOTO RX_EXIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫_3:
    SPEED 4
    MOVE G6B, 130, 10, 50
    MOVE G6C, 130, 10, 50
    WAIT

    'DELAY 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    RETURN
    'GOTO RX_EXIT
    '*****************************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й╣О©╫О©╫О©╫1:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й╣О©╫О©╫О©╫1_LOOP:
    '  MOVE G6B, 190, 10, 50
    ' MOVE G6C, 190, 10, 50  ,   ,
    WAIT
    DELAY 3
    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 12
        MOVE G6A,102,  73, 145,  93, 100, 100
        MOVE G6D,102,  79, 145,  93, 100, 100
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,100,  79, 145,  93, 100, 100
        WAIT


        SPEED 5
        MOVE G6A,102,  84, 145,  78, 100, 100
        MOVE G6D,102,  68, 145,  108, 100, 100
        MOVE G6A,100,  84, 145,  78, 100, 100
        MOVE G6D,100,  68, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6A,90,  90, 145,  78, 102, 100
        MOVE G6D,104,  71, 145,  105, 100, 100
        WAIT
        SPEED 6
        MOVE G6A,90,  80, 130, 102, 104
        MOVE G6D,105,  76, 146,  93,  100
        WAIT
    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 12
        MOVE G6A,102,  73, 145,  93, 100, 100
        MOVE G6D,102,  79, 145,  93, 100, 100
        MOVE G6A,100,  73, 145,  93, 100, 100
        MOVE G6D,100,  79, 145,  93, 100, 100
        WAIT


        SPEED 5
        MOVE G6A,102,  88, 145,  78, 100, 100
        MOVE G6D,102,  65, 145,  108, 100, 100
        MOVE G6A,100,  88, 145,  78, 100, 100
        MOVE G6D,100,  65, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6A,104,  86, 146,  80, 100, 100
        MOVE G6D,90,  58, 145,  110, 100, 100
        WAIT
        SPEED 6
        MOVE G6D,90,  85, 130, 98, 104
        MOVE G6A,105,  77, 146,  93,  100
        WAIT
    ENDIF
    SPEED 10
    'GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    GOTO RX_EXIT
    '*************************************************

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й╣О©╫О©╫О©╫1:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й╣О©╫О©╫О©╫1_LOOP:
    '   MOVE G6B, 190, 10, 50
    '  MOVE G6C, 190, 10, 50
    WAIT
    DELAY 3

    IF О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0 THEN
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 1
        SPEED 12
        MOVE G6D,102,  73, 145,  93, 100, 100
        MOVE G6A,102,  79, 145,  93, 100, 100
        WAIT

        SPEED 5
        MOVE G6D,102,  84, 145,  78, 100, 100
        MOVE G6A,102,  68, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6D,90,  90, 145,  78, 102, 100
        MOVE G6A,104,  71, 145,  105, 100, 100
        WAIT
        SPEED 6
        MOVE G6D,90,  80, 130, 102, 104
        MOVE G6A,105,  76, 146,  93,  100
        WAIT



    ELSE
        О©╫О©╫О©╫О©╫О©╫О©╫О©╫ = 0
        SPEED 12
        MOVE G6D,102,  73, 145,  93, 100, 100
        MOVE G6A,102,  79, 145,  93, 100, 100
        WAIT


        SPEED 5
        MOVE G6D,102,  88, 145,  78, 100, 100
        MOVE G6A,102,  65, 145,  108, 100, 100
        WAIT

        SPEED 7
        MOVE G6D,104,  86, 146,  80, 100, 100
        MOVE G6A,90,  58, 145,  110, 100, 100
        WAIT

        SPEED 6
        MOVE G6A,90,  85, 130, 98, 104
        MOVE G6D,105,  77, 146,  93,  100
        WAIT



    ENDIF

    SPEED 10
    '    GOSUB О©╫Б╨╩О©╫з╪О©╫2
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT

    GOTO RX_EXIT


О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1:
    О©╫О©╫О©╫О©╫с╣О©╫ = 8
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    'GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫18О©╫О©╫
    'DELAY 20
    SPEED 6
    GOSUB All_motor_mode3
    MOVE G6B, 190, 10, 50
    MOVE G6C, 190, 10, 50
    WAIT

    DELAY 20
    'HIGHSPEED SETON

    SPEED 6
    MOVE G6D,  90,  74, 144,  94, 109
    MOVE G6A, 108,  76, 146,  93, 96
    WAIT

    SPEED 8
    MOVE G6D,90, 90, 120, 101, 109,99
    MOVE G6A,108,  76, 147,  92,  96,100
    WAIT

    'HIGHSPEED SETOFF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1_2	

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1_2:
    MOVE G6A,110,  76, 147,  93, 100,100
    MOVE G6D,96, 90, 120, 101, 106,99
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1_3:
    'ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 90,  56, 145, 114, 109
    MOVE G6A,108,  76, 147,  89,  95
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,108,  76, 147, 89,  97
    MOVE G6A,90, 100, 142,  68, 107
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1_4
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_1_4:
    SPEED 9
    MOVE G6A,95, 90, 120, 101, 110,99
    MOVE G6D,108,  76, 146,  92,  95,99
    WAIT

    SPEED 7
    MOVE G6A,100,  76, 145,  92, 99, 100
    MOVE G6D,100,  76, 145,  92, 99, 100
    WAIT

    RETURN

    '*******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2:
    О©╫О©╫О©╫О©╫с╣О©╫ = 8
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    'GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫18О©╫О©╫
    'DELAY 20
    SPEED 6
    GOSUB All_motor_mode3
    MOVE G6B, 150, 10, 50
    MOVE G6C, 150, 10, 50
    WAIT

    DELAY 20
    'HIGHSPEED SETON

    SPEED 6
    MOVE G6D,  90,  74, 144,  94, 109
    MOVE G6A, 108,  76, 146,  93, 96
    WAIT

    SPEED 8
    MOVE G6D,90, 90, 120, 101, 109,99
    MOVE G6A,108,  76, 147,  92,  96,100
    WAIT

    'HIGHSPEED SETOFF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_2	

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_2:
    MOVE G6A,110,  76, 147,  93, 100,100
    MOVE G6D,96, 90, 120, 101, 106,99
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_3:
    'ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 90,  56, 145, 114, 109
    MOVE G6A,108,  76, 147,  89,  95
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,108,  76, 147, 89,  97
    MOVE G6A,90, 100, 142,  68, 107
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_4
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_2_4:
    SPEED 9
    MOVE G6A,95, 90, 120, 101, 110,99
    MOVE G6D,108,  76, 146,  92,  95,99
    WAIT

    SPEED 7
    MOVE G6A,100,  76, 145,  92, 99, 100
    MOVE G6D,100,  76, 145,  92, 99, 100
    WAIT

    RETURN
    '*******************************************
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_3:
    О©╫О©╫О©╫О©╫с╣О©╫ = 8
    О©╫б©О©╫с╣О©╫ = 4
    О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0

    'GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫18О©╫О©╫
    'DELAY 20
    SPEED 6
    GOSUB All_motor_mode3
    MOVE G6B, 130, 10, 50
    MOVE G6C, 130, 10, 50
    WAIT

    DELAY 20
    'HIGHSPEED SETON

    SPEED 6
    MOVE G6D,  90,  74, 144,  94, 109
    MOVE G6A, 108,  76, 146,  93, 96
    WAIT

    SPEED 8
    MOVE G6D,90, 90, 120, 101, 109,99
    MOVE G6A,108,  76, 147,  92,  96,100
    WAIT

    'HIGHSPEED SETOFF
    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_3_2	

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_3_2:
    MOVE G6A,110,  76, 147,  93, 100,100
    MOVE G6D,96, 90, 120, 101, 106,99
    WAIT

О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_3_3:
    'ETX 4800,13 'О©╫О©╫О©╫О©╫О©╫з╣Е╦╕ О©╫О©╫О©╫О©╫

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    MOVE G6D, 90,  56, 145, 114, 109
    MOVE G6A,108,  76, 147,  89,  95
    WAIT

    SPEED О©╫б©О©╫с╣О©╫
    MOVE G6D,108,  76, 147, 89,  97
    MOVE G6A,90, 100, 142,  68, 107
    WAIT

    SPEED О©╫О©╫О©╫О©╫с╣О©╫

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    IF О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 1 THEN
        О©╫я╬О©╫О©╫О©╫х╝О©╫О©╫ = 0
        GOTO MAIN
    ENDIF


    ERX 4800,A, О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_3_4
    IF A = 11 THEN
        GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_3_4
        '    ELSE
        '    	MOVE G6A, 90, 100, 100, 115, 110,100
        ' 		MOVE G6D,112,  76, 146,  93,  96,100
        ' 		MOVE G6B,90
        ' 		MOVE G6C,110
        ' 		WAIT
        ' 		HIGHSPEED SETOFF
        ' 		SPEED 8

        ' 		MOVE G6D, 106,  76, 146,  93,  96,100		
        ' 		MOVE G6A,  88,  71, 152,  91, 106,100
        ' 		MOVE G6C, 100
        ' 		MOVE G6B, 100
        ' 		WAIT	
        ' 		SPEED 8
        ' 		GOSUB О©╫Б╨╩О©╫з╪О©╫2

        ' 		GOTO RX_EXIT
    ENDIF
О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫х╠О©╫_3_4:
    SPEED 9
    MOVE G6A,95, 90, 120, 101, 110,99
    MOVE G6D,108,  76, 146,  92,  95,99
    WAIT

    SPEED 7
    MOVE G6A,100,  76, 145,  92, 99, 100
    MOVE G6D,100,  76, 145,  92, 99, 100
    WAIT

    RETURN

    '******************************************
О©╫О©╫э©ч╧ъ©О©╫О©╫О©╫О©╫О©╫1cm:
    GOSUB All_motor_mode3

    SPEED 4
    MOVE G6A, 88,  71, 152,  91, 110
    MOVE G6D,108,  77, 146,  93,  94
    MOVE G6B,100,40
    MOVE G6C,100,40
    WAIT

    SPEED 8
    MOVE G6A, 90, 100, 110, 100, 114
    MOVE G6D,114,  78, 146,  93,  94
    WAIT

    GOSUB Leg_motor_mode2

    SPEED 8
    MOVE G6A, 90, 140, 35, 130, 114
    MOVE G6D,114,  71, 155,  90,  94
    WAIT


    SPEED 12
    MOVE G6A,  80, 55, 130, 140, 114,
    MOVE G6D,114,  70, 155,  90,  94
    WAIT

    GOSUB Leg_motor_mode3

    SPEED 7
    MOVE G6A, 105, 75, 100, 155, 100,
    MOVE G6D,95,  90, 165,  70, 100
    MOVE G6B,160,50
    MOVE G6C,160,40
    WAIT

    SPEED 6
    MOVE G6A, 114, 90, 90, 155,100,
    MOVE G6D,95,  100, 165,  65, 105
    MOVE G6B,180,50
    MOVE G6C,180,30
    WAIT

    '****************************
    GOSUB Leg_motor_mode2	
    SPEED 8
    MOVE G6A, 114, 90, 100, 150,95,
    MOVE G6D,95,  90, 165,  70, 105
    WAIT

    SPEED 12
    MOVE G6A, 114, 90, 100, 150,95,
    MOVE G6D,90,  120, 40,  140, 108
    WAIT

    SPEED 10
    MOVE G6A, 114, 90, 110, 130,95,
    MOVE G6D,90,  95, 90,  145, 108
    MOVE G6B,140,50
    MOVE G6C,140,30
    WAIT

    SPEED 10
    MOVE G6A, 110, 90, 110, 130,95,
    MOVE G6D,80,  85, 110,  135, 108
    MOVE G6B,110,40
    MOVE G6C,110,40
    WAIT

    SPEED 5
    MOVE G6D, 98, 90, 110, 125,99,
    MOVE G6A,98,  90, 110,  125, 99
    MOVE G6B,110,40
    MOVE G6C,110,40
    WAIT

    SPEED 6
    MOVE G6A,100,  77, 145,  93, 100, 100
    MOVE G6D,100,  77, 145,  93, 100, 100
    MOVE G6B,100,  30,  80
    MOVE G6C,100,  30,  80
    WAIT

    GOTO RX_EXIT
    '****************************************

О©╫О©╫э©О©╫О©╫О©╫О©╫ъ©О©╫О©╫О©╫О©╫О©╫1cm:
    GOSUB All_motor_mode3
    GOSUB All_motor_mode3

    SPEED 4
    MOVE G6D, 88,  71, 152,  91, 110
    MOVE G6A,108,  77, 146,  93,  94
    MOVE G6B,100,40
    MOVE G6C,100,40
    WAIT

    SPEED 8
    MOVE G6D, 90, 100, 110, 100, 114
    MOVE G6A,113,  78, 146,  93,  94
    WAIT

    GOSUB Leg_motor_mode2

    SPEED 8
    MOVE G6D, 90, 140, 35, 130, 114
    MOVE G6A,113,  71, 155,  90,  94
    WAIT


    SPEED 12
    MOVE G6D,  80, 55, 130, 140, 114,
    MOVE G6A,113,  70, 155,  90,  94
    WAIT

    GOSUB Leg_motor_mode3

    SPEED 7
    MOVE G6D, 105, 75, 100, 155, 100,
    MOVE G6A,95,  90, 165,  70, 100
    MOVE G6C,160,50
    MOVE G6B,160,40
    WAIT

    SPEED 6
    MOVE G6D, 113, 90, 90, 155,100,
    MOVE G6A,95,  100, 165,  65, 105
    MOVE G6C,180,50
    MOVE G6B,180,30
    WAIT

    '****************************
    GOSUB Leg_motor_mode2	
    SPEED 8
    MOVE G6D, 114, 90, 100, 150,95,
    MOVE G6A,95,  90, 165,  70, 105
    WAIT

    SPEED 12
    MOVE G6D, 114, 90, 100, 150,95,
    MOVE G6A,90,  120, 40,  140, 108
    WAIT

    SPEED 10
    MOVE G6D, 114, 90, 110, 130,95,
    MOVE G6A,90,  95, 90,  145, 108
    MOVE G6C,140,50
    MOVE G6B,140,30
    WAIT

    SPEED 10
    MOVE G6D, 110, 90, 110, 130,95,
    MOVE G6A,80,  85, 110,  135, 108
    MOVE G6B,110,40
    MOVE G6C,110,40
    WAIT

    SPEED 5
    MOVE G6A, 98, 90, 110, 125,99,
    MOVE G6D,98,  90, 110,  125, 99
    MOVE G6B,110,40
    MOVE G6C,110,40
    WAIT

    SPEED 6
    MOVE G6A,100,  77, 145,  93, 100, 100
    MOVE G6D,100,  77, 145,  93, 100, 100
    MOVE G6B,100,  30,  80
    MOVE G6C,100,  30,  80
    WAIT

    GOTO RX_EXIT

    '******************************************
<<<<<<< HEAD

О©╫О©╫э©О©╫О©╫О©╫О©╫ъЁО©╫О©╫О©╫О©╫О©╫1cm:
=======
╟Х╢э©ю╦╔╧ъЁ╩╦╝╠Б1cm:
    
>>>>>>> f20e10f (Chore: К╙╗Л┘≤ М▄▄Л²╪Л≈░ ЙЁ└К▀╗ К┌╢К═╓Й╟─К┼■ К╙╗Л┘≤ Л╤■Й╟─)
    GOSUB All_motor_mode3

    '©ю╦╔╧ъ ©оюЭ аЩ╬НЁж╠Б ''''
    SPEED 5
    MOVE G6A, 100, 110,  112, 92,  101, 100
    MOVE G6D,  100,  112, 112, 92, 101, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ╦Я ©чбйю╦╥н
    SPEED 5
    MOVE G6A, 112, 110,  112, 92,  101, 100
    MOVE G6D,  85,  110, 112, 92, 108, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT 	

    '©ю╦╔╧ъ ╟еюг╧ъ╦Я╠НаЖ   аЩ╬НЁж╠Б
    SPEED 5
    MOVE G6A, 112, 110,  112, 92,  101, 100
    MOVE G6D,  95,  110, 112, 92, 108, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT 	

    '©ю╦╔╧ъ ╟еюг  аЩ╬НЁж╠Б
    SPEED 5
    MOVE G6A, 112, 110,  112, 92,  101, 100
    MOVE G6D,  95,  105, 63, 119, 116, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ аЩ╬НЁж╠Б
    SPEED 5
    MOVE G6A, 112, 110,  112, 92,  101, 100
    MOVE G6D,  95,  15, 139, 139, 116, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧╚╦╜╪ЖаВ цжа╬╣И╠Б
    SPEED 2
    MOVE G6A, 112, 110,  112, 77,  91, 100
    MOVE G6D,  95,  15, 169, 139, 116, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧╚╦╜ ╦╧юл╣И╠Б
    SPEED 1
    MOVE G6A, 112, 125,  102, 65,  91, 100
    MOVE G6D,  95,  15, 169, 139, 116, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT	

    '©ч╧╚╦╜ ╣И╠Б
    SPEED 5
    MOVE G6A, 108, 140,  92, 82,  91, 100
    MOVE G6D,  95,  15, 169, 139, 116, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '╫цгХ ©ю╦╔╧ъ╥н аЖейго╟М Ё╩╥а╟║╢б ╣©юш
    SPEED 5
    MOVE G6A, 105, 140,  92, 102,  81, 100
    MOVE G6D,  95,  15, 169, 149, 116, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ , ©ю╦╔╧ъ ╪╪©Л╠Б
    SPEED 5
    MOVE G6A, 105, 120,  112, 102,  81, 100
    MOVE G6D,  95,  35, 149, 139, 116, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╟еюг ╪ЖаВю╦╥н ╪╪©Л╠Б ';';'
    SPEED 5
    MOVE G6A, 105, 120,  112, 102,  96, 100
    MOVE G6D,  100,  35, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╧ъ╦Я ©чбйю╦╥н ╠Б©Оюл╠Б
    SPEED 5
    MOVE G6A, 97, 120,  112, 102,  96, 100
    MOVE G6D,  105,  35, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╧╚╦╜╟З ╩Сц╪ ╪Вюл╠Б
    SPEED 3
    MOVE G6A, 97, 120,  112, 102,  96, 100
    MOVE G6D,  110,  45, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╧╚╦╜ ╬Ю╟ё ©ц╦╝╠Б
    SPEED 5
    MOVE G6A, 97, 120,  102, 107,  96, 100
    MOVE G6D,  110,  55, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╧╚╦╜ ╬Ю╟ё ©ц╦╝╠Б (©ю╦╔╧ъ ╧ъ╦Я аъ╫и юБ╠Б)
    SPEED 5
    MOVE G6A, 97, 120,  102, 107,  96, 100
    MOVE G6D,  112,  55, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╬ую╦╥н ╟╝╟М©ю╠Б
    SPEED 3
    MOVE G6A, 97, 105,  103, 132,  96, 100
    MOVE G6D,  112,  55, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╬ую╦╥н ╟╝╟М©ю╠Б 2
    SPEED 3
    MOVE G6A, 97, 110,  97, 160,  96, 100
    MOVE G6D,  112,  55, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ ╬ую╦╥н ╟╝╟М©ю╠Б 3
    SPEED 3
    MOVE G6A, 97, 110,  107, 160,  96, 100
    MOVE G6D,  112,  55, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ Ё╩╦╝╠Б1
    SPEED 3
    MOVE G6A, 90, 65,  149, 149,  96, 100
    MOVE G6D,  112,  55, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ Ё╩╦╝╠Б2
    SPEED 3
    MOVE G6A, 90, 65,  149, 149,  96, 100
    MOVE G6D,  107,  55, 149, 139, 106, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ Ё╩╦╝╠Б3
    SPEED 3
    MOVE G6A, 97, 55,  149, 136,  96, 100
    MOVE G6D,  107,  55, 149, 139, 104, 100
    MOVE G6B, 101,  36,  85, 100, 100, 101
    MOVE G6C,  99,  32,  92, 100,  95, 100
    WAIT


    SPEED 4
    GOSUB ╠Б╨╩юз╪╪''╟к╪Ж ╢К╩С

    ETX 4800, 254

    RETURN
    '******************************************
<<<<<<< HEAD
О©╫О©╫э©ч╧ъЁО©╫О©╫О©╫О©╫О©╫1cm:
    GOSUB All_motor_mode3
=======
╟Х╢э©ч╧ъЁ╩╦╝╠Б1cm:
    
	GOSUB All_motor_mode3

    '©ч╧ъ ©оюЭ аЩ╬НЁж╠Б ''''
    SPEED 5
    MOVE G6D, 100, 110,  112, 92,  101, 100
    MOVE G6A,  100,  112, 112, 92, 101, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ╦Я ©чбйю╦╥н
    SPEED 5
    MOVE G6D, 112, 110,  112, 92,  101, 100
    MOVE G6A,  85,  110, 112, 92, 108, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT 	

    '©ч╧ъ ╟еюг╧ъ╦Я╠НаЖ   аЩ╬НЁж╠Б
    SPEED 5
    MOVE G6D, 112, 110,  112, 92,  101, 100
    MOVE G6A,  95,  110, 112, 92, 108, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT 	

    '©ч╧ъ ╟еюг  аЩ╬НЁж╠Б
    SPEED 5
    MOVE G6D, 112, 110,  112, 92,  101, 100
    MOVE G6A,  95,  105, 63, 119, 116, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ аЩ╬НЁж╠Б
    SPEED 5
    MOVE G6D, 112, 110,  112, 92,  101, 100
    MOVE G6A,  95,  15, 139, 139, 116, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧╚╦╜╪ЖаВ цжа╬╣И╠Б
    SPEED 2
    MOVE G6D, 112, 110,  112, 77,  91, 100
    MOVE G6A,  95,  15, 169, 139, 116, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧╚╦╜ ╦╧юл╣И╠Б
    SPEED 1
    MOVE G6D, 112, 125,  102, 65,  91, 100
    MOVE G6A,  95,  15, 169, 139, 116, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT	

    '©ю╦╔╧╚╦╜ ╣И╠Б
    SPEED 5
    MOVE G6D, 108, 140,  92, 82,  91, 100
    MOVE G6A,  95,  15, 169, 139, 116, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '╫цгХ ©ч╧ъ╥н аЖейго╟М Ё╩╥а╟║╢б ╣©юш
    SPEED 5
    MOVE G6D, 105, 140,  92, 102,  81, 100
    MOVE G6A,  95,  15, 169, 149, 116, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ю╦╔╧ъ , ©ч╧ъ ╪╪©Л╠Б
    SPEED 5
    MOVE G6D, 105, 120,  112, 102,  81, 100
    MOVE G6A,  95,  35, 149, 139, 116, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╟еюг ╪ЖаВю╦╥н ╪╪©Л╠Б ';';'
    SPEED 5
    MOVE G6D, 105, 120,  112, 102,  96, 100
    MOVE G6A,  100,  35, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╧ъ╦Я ©чбйю╦╥н ╠Б©Оюл╠Б
    SPEED 5
    MOVE G6D, 97, 120,  112, 102,  96, 100
    MOVE G6A,  105,  35, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╧╚╦╜╟З ╩Сц╪ ╪Вюл╠Б
    SPEED 3
    MOVE G6D, 97, 120,  112, 102,  96, 100
    MOVE G6A,  110,  45, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╧╚╦╜ ╬Ю╟ё ©ц╦╝╠Б
    SPEED 5
    MOVE G6D, 97, 120,  102, 107,  96, 100
    MOVE G6A,  110,  55, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╧╚╦╜ ╬Ю╟ё ©ц╦╝╠Б (©ч╧ъ ╧ъ╦Я аъ╫и юБ╠Б)
    SPEED 5
    MOVE G6D, 97, 120,  102, 107,  96, 100
    MOVE G6A,  112,  55, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╬ую╦╥н ╟╝╟М©ю╠Б
    SPEED 3
    MOVE G6D, 97, 105,  103, 132,  96, 100
    MOVE G6A,  112,  55, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╬ую╦╥н ╟╝╟М©ю╠Б 2
    SPEED 3
    MOVE G6D, 97, 110,  97, 160,  96, 100
    MOVE G6A,  112,  55, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ ╬ую╦╥н ╟╝╟М©ю╠Б 3
    SPEED 3
    MOVE G6D, 97, 110,  107, 160,  96, 100
    MOVE G6A,  112,  55, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ Ё╩╦╝╠Б1
    SPEED 3
    MOVE G6D, 90, 65,  149, 149,  96, 100
    MOVE G6A,  112,  55, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ Ё╩╦╝╠Б2
    SPEED 3
    MOVE G6D, 90, 65,  149, 149,  96, 100
    MOVE G6A,  107,  55, 149, 139, 106, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

    '©ч╧ъ Ё╩╦╝╠Б3
    SPEED 3
    MOVE G6D, 97, 55,  149, 136,  96, 100
    MOVE G6A,  107,  55, 149, 139, 104, 100
    MOVE G6C, 101,  36,  85, 100, 100, 101
    MOVE G6B,  99,  32,  92, 100,  95, 100
    WAIT

>>>>>>> f20e10f (Chore: К╙╗Л┘≤ М▄▄Л²╪Л≈░ ЙЁ└К▀╗ К┌╢К═╓Й╟─К┼■ К╙╗Л┘≤ Л╤■Й╟─)

    SPEED 4
    GOSUB ╠Б╨╩юз╪╪''╟к╪Ж ╢К╩С

    ETX 4800, 254

    RETURN

    '******************************************

╩Ч╩Х╩Ч╩Х:
    GOSUB All_motor_mode3
    ╨╦гЮCOUNT = 0
    SPEED 13
    'HIGHSPEED╥н ╬х го╦И ╣з╥н ╟╗... ╧лдёЁПюн ╣М
    'HIGHSPEED SETON
    '..... юл╟е го╦И ╢ыю╫ ╦П╪г╣И╠НаЖ ╢ы ╟Ё╩║╤СаЖ╢б╣╔ ╨╧╠м╦╕ ╬Н╤╩╟т го╢баЖ ╦П╦ё╟зю╫


    IF ╨╦гЮ╪Ь╪╜ = 0 THEN
        ╨╦гЮ╪Ь╪╜ = 1
        MOVE G6A,95,  76, 147,  93, 101
        MOVE G6D,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO ╩Ч╩Х╩Ч╩Х1
    ELSE
        ╨╦гЮ╪Ь╪╜ = 0
        MOVE G6D,95,  76, 147,  93, 101
        MOVE G6A,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO ╩Ч╩Х╩Ч╩Х4
    ENDIF


    '**********************

╩Ч╩Х╩Ч╩Х1: '©ч╧ъ
    'HIGHSPEED SETON
    MOVE G6D,104,  77, 147, 93, 100
    MOVE G6A,95,  95, 143,  94,  102
    MOVE G6B, 100
    MOVE G6C, 100
    WAIT
╩Ч╩Х╩Ч╩Х2:

    MOVE G6A,99,    75, 146, 97,  98
    MOVE G6D, 95,  77, 147,  90, 100
    WAIT

    GOSUB ╬у╣з╠Б©О╠БцЬа╓
    IF Ёя╬НаЬх╝юн = 1 THEN
        Ёя╬НаЬх╝юн = 0

        GOTO RX_EXIT
    ENDIF

    ' ╨╦гЮCOUNT = ╨╦гЮCOUNT + 1
    'IF ╨╦гЮCOUNT > ╨╦гЮх╫╪Ж THEN  GOTO юЭаЬа╬а╬╟ию╫_2_stop

    ERX 4800,A, ╩Ч╩Х╩Ч╩Х4
    IF A <> A_old THEN
╩Ч╩Х╩Ч╩Х_2_stop:
        MOVE G6D,95,  87, 143, 97, 102
        MOVE G6A,104,  76, 145,  92,  100
        MOVE G6C, 100
        MOVE G6B,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB ╬ха╓х╜юз╪╪
        SPEED 5
        GOSUB ╠Б╨╩юз╪╪2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*********************************

╩Ч╩Х╩Ч╩Х4: '©ю╦╔╧ъ
    MOVE G6A,104,  77, 147, 93, 100
    MOVE G6D,95,  95, 143,  94,  102
    MOVE G6C, 100
    MOVE G6B, 100
    WAIT

╩Ч╩Х╩Ч╩Х5:
    MOVE G6D,99,    75, 146, 97,  98
    MOVE G6A, 95,  77, 147,  93, 100
    WAIT


    GOSUB ╬у╣з╠Б©О╠БцЬа╓
    IF Ёя╬НаЬх╝юн = 1 THEN
        Ёя╬НаЬх╝юн = 0
        GOTO RX_EXIT
    ENDIF

    ' ╨╦гЮCOUNT = ╨╦гЮCOUNT + 1
    ' IF ╨╦гЮCOUNT > ╨╦гЮх╫╪Ж THEN  GOTO юЭаЬа╬а╬╟ию╫_5_stop

    ERX 4800,A, ╩Ч╩Х╩Ч╩Х1
    IF A <> A_old THEN
╩Ч╩Х╩Ч╩Х5_stop:
        MOVE G6A,95,  87, 143, 97, 102
        MOVE G6D,104,  76, 145,  92,  100
        MOVE G6B, 100
        MOVE G6C,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB ╬ха╓х╜юз╪╪
        SPEED 5
        GOSUB ╠Б╨╩юз╪╪2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF
		GOTO ╩Ч╩Х╩Ч╩Х1
    '******************************************

х╫╪Ж_╩Ч╩Х╩Ч╩Х:
    GOSUB All_motor_mode3
    ╨╦гЮCOUNT = 0
    SPEED 13
    'HIGHSPEED╥н ╬х го╦И ╣з╥н ╟╗... ╧лдёЁПюн ╣М
    'HIGHSPEED SETON
    '..... юл╟е го╦И ╢ыю╫ ╦П╪г╣И╠НаЖ ╢ы ╟Ё╩║╤СаЖ╢б╣╔ ╨╧╠м╦╕ ╬Н╤╩╟т го╢баЖ ╦П╦ё╟зю╫


    IF ╨╦гЮ╪Ь╪╜ = 0 THEN
        ╨╦гЮ╪Ь╪╜ = 1
        MOVE G6A,95,  76, 147,  93, 101
        MOVE G6D,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO ╩Ч╩Х╩Ч╩Х1
    ELSE
        ╨╦гЮ╪Ь╪╜ = 0
        MOVE G6D,95,  76, 147,  93, 101
        MOVE G6A,101,  76, 147,  93, 98
        MOVE G6B,100
        MOVE G6C,100
        WAIT

        GOTO ╩Ч╩Х╩Ч╩Х4
    ENDIF


    '**********************

х╫╪Ж_╩Ч╩Х╩Ч╩Х1: '©ч╧ъ
    'HIGHSPEED SETON
    MOVE G6D,104,  77, 147, 93, 100
    MOVE G6A,95,  95, 143,  94,  102
    MOVE G6B, 100
    MOVE G6C, 100
    WAIT
х╫╪Ж_╩Ч╩Х╩Ч╩Х2:

    MOVE G6A,99,    75, 146, 97,  98
    MOVE G6D, 95,  77, 147,  90, 100
    WAIT

    GOSUB ╬у╣з╠Б©О╠БцЬа╓
    IF Ёя╬НаЬх╝юн = 1 THEN
        Ёя╬НаЬх╝юн = 0

        GOTO RX_EXIT
    ENDIF

    ╨╦гЮCOUNT = ╨╦гЮCOUNT + 1
    IF ╨╦гЮCOUNT > ╨╦гЮх╫╪Ж THEN  GOTO х╫╪Ж_╩Ч╩Х╩Ч╩Х_2_stop

    ERX 4800,A, х╫╪Ж_╩Ч╩Х╩Ч╩Х4
    IF A <> A_old THEN
х╫╪Ж_╩Ч╩Х╩Ч╩Х_2_stop:
        MOVE G6D,95,  87, 143, 97, 102
        MOVE G6A,104,  76, 145,  92,  100
        MOVE G6C, 100
        MOVE G6B,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB ╬ха╓х╜юз╪╪
        SPEED 5
        GOSUB ╠Б╨╩юз╪╪2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF

    '*********************************

х╫╪Ж_╩Ч╩Х╩Ч╩Х4: '©ю╦╔╧ъ
    MOVE G6A,104,  77, 147, 93, 100
    MOVE G6D,95,  95, 143,  94,  102
    MOVE G6C, 100
    MOVE G6B, 100
    WAIT

х╫╪Ж_╩Ч╩Х╩Ч╩Х5:
    MOVE G6D,99,    75, 146, 97,  98
    MOVE G6A, 95,  77, 147,  93, 100
    WAIT


    GOSUB ╬у╣з╠Б©О╠БцЬа╓
    IF Ёя╬НаЬх╝юн = 1 THEN
        Ёя╬НаЬх╝юн = 0
        GOTO RX_EXIT
    ENDIF

    ╨╦гЮCOUNT = ╨╦гЮCOUNT + 1
    IF ╨╦гЮCOUNT > ╨╦гЮх╫╪Ж THEN  GOTO х╫╪Ж_╩Ч╩Х╩Ч╩Х5_stop

    ERX 4800,A, х╫╪Ж_╩Ч╩Х╩Ч╩Х1
    IF A <> A_old THEN
х╫╪Ж_╩Ч╩Х╩Ч╩Х5_stop:
        MOVE G6A,95,  87, 143, 97, 102
        MOVE G6D,104,  76, 145,  92,  100
        MOVE G6B, 100
        MOVE G6C,100
        WAIT
        HIGHSPEED SETOFF
        SPEED 15
        GOSUB ╬ха╓х╜юз╪╪
        SPEED 5
        GOSUB ╠Б╨╩юз╪╪2

        'DELAY 400
        GOTO RX_EXIT
    ENDIF
		GOTO ╩Ч╩Х╩Ч╩Х1


    '******************************************


MAIN: 'О©╫С╨╖╪О©╫О©╫О©╫

    ETX 4800, 200 ' О©╫О©╫О©╫О©╫ О©╫О©╫О©╫О©╫ х╝О©╫О©╫ О©╫ш╫О©╫ О©╫О©╫

MAIN_2:

    GOSUB О©╫у╣з╠О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    GOSUB О©╫б©О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    GOSUB О©╫О©╫О©╫э╪О©╫О©╫е╦О©╫О©╫О©╫О©╫О©╫х╝О©╫О©╫


    ERX 4800,A,MAIN_2	

    A_old = A

    '**** О©╫т╥б╣О©╫ AО©╫О©╫О©╫О©╫ 0 О©╫л╦О©╫ MAIN О©╫С╨╖╥О©╫ О©╫О©╫О©╫О©╫
    '**** 1О©╫л╦О©╫ 	 О©╫О©╫, 2О©╫л╦О©╫ key2О©╫О©╫... О©╫О©╫О©╫б╧О©╫
    ON A GOTO MAIN,KEY1,KEY2,KEY3,KEY4,KEY5,KEY6,KEY7,KEY8,KEY9,KEY10,KEY11,KEY12,KEY13,KEY14,KEY15,KEY16,KEY17,KEY18 ,KEY19,KEY20,KEY21,KEY22,KEY23,KEY24,KEY25,KEY26,KEY27,KEY28 ,KEY29,KEY30,KEY31,KEY32


    IF A > 100 AND A < 110 THEN
        BUTTON_NO = A - 100
        GOSUB Number_Play
        'GOSUB SOUND_PLAY_CHK
        GOSUB GOSUB_RX_EXIT


    ELSEIF A = 250 THEN
        GOSUB All_motor_mode3
        SPEED 4
        MOVE G6A,100,  76, 145,  93, 100, 100
        MOVE G6D,100,  76, 145,  93, 100, 100
        MOVE G6B,100,  40,  90,
        MOVE G6C,100,  40,  90,
        WAIT
        DELAY 500
        SPEED 6
        GOSUB О©╫Б╨╩О©╫з╪О©╫

    ENDIF


    GOTO MAIN	
    '*******************************************
    '		MAIN О©╫С╨╖╥О©╫ О©╫О©╫О©╫О©╫
    '*******************************************

KEY1:
    ETX  4800,1

    ' О©╫О©╫О©╫О©╫х╫О©╫О©╫ = 2
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫10
    GOTO RX_EXIT
    '***************	
KEY2:
    ETX  4800,2

<<<<<<< HEAD
    О©╫О©╫О©╫О©╫х╫О©╫О©╫ = 6
    GOTO х╫О©╫О©╫_О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫
=======
    ╨╦гЮх╫╪Ж = 1
    GOTO х╫╪Ж_юЭаЬа╬а╬╟ию╫
>>>>>>> 77312ab (Add: Л╢╛Л≤│.bas Л╤■Й╟─)

    GOTO RX_EXIT
    '***************

KEY3:
    ETX 4800, 3
    ' GOTO О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫20
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫10
    GOTO RX_EXIT
KEY4:
    ETX 4800, 4
<<<<<<< HEAD
    GOSUB О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫20
    '  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3
=======
    GOSUB ©чбй©╥ю╦╥н20
>>>>>>> 555d29a (Chore: Л═└К╟╘М∙≤М√╔ Л└╦К╤─Л═│Л°╪К║° Л╤■Й╟─)
    GOTO RX_EXIT
    '***************
KEY5:
    ETX  4800,5

    'J = AD(О©╫О©╫О©╫э╪О©╫ADО©╫О©╫ф╝)	'О©╫О©╫О©╫э╪О©╫О©╫е╦О©╫О©╫О©╫ О©╫п╠О©╫
    'ETX 4800, J
    'BUTTON_NO = J
    'GOSUB Number_Play
    'GOSUB SOUND_PLAY_CHK
    'GOSUB GOSUB_RX_EXIT
    GOSUB О©╫с╦О©╫О©╫б©О©╫О©╫ъ╬О©╫

    GOTO RX_EXIT
    '***************
KEY6:
    ETX 4800, 6
    GOSUB О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫20
    GOTO RX_EXIT
KEY7:
    ETX 4800, 7
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫45
    GOTO RX_EXIT
    '***************
KEY8:
    ETX  4800,8
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY9:
    ETX 4800, 9
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45
    GOTO RX_EXIT
    '***************
KEY10: '0
    ETX 4800, 10
    GOSUB О©╫Б╨╩О©╫з╪О©╫
    GOTO RX_EXIT
    '***************
KEY11: ' О©╫О©╫
    ETX  4800,11

    GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫
    GOTO RX_EXIT

    '***************
KEY12: ' О©╫О©╫
    ETX  4800,12

    GOSUB О©╫О©╫О©╫гЁО©╫О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY13: 'О©╫О©╫
    ETX  4800,13
    'GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫90О©╫О©╫
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й╣О©╫О©╫О©╫1

    GOTO RX_EXIT
    '**************
KEY14: ' О©╫О©╫
    ETX  4800,14
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й╣О©╫О©╫О©╫1
    GOTO RX_EXIT


    GOTO RX_EXIT
    '***************
KEY15: 'A
    ETX 4800, 15
    GOSUB О©╫с╦О©╫О©╫О©╫О©╫О©╫30О©╫О©╫
    GOTO RX_EXIT
KEY16: ' POWER
    ETX  4800,16

    GOSUB Leg_motor_mode3
    IF MODE = 0 THEN
        SPEED 10
        MOVE G6A,100, 140,  37, 145, 100, 100
        MOVE G6D,100, 140,  37, 145, 100, 100
        WAIT
    ENDIF
    SPEED 4
    GOSUB О©╫О©╫О©╫О©╫О©╫з╪О©╫	
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫

    GOSUB MOTOR_GET
    GOSUB MOTOR_OFF


    GOSUB GOSUB_RX_EXIT
KEY16_1:

    IF О©╫О©╫О©╫О©╫ONOFF = 1  THEN
        OUT 52,1
        DELAY 200
        OUT 52,0
        DELAY 200
    ENDIF
    ERX 4800,A,KEY16_1
    ETX  4800,A

    '**** RX DATA Number Sound ********
    BUTTON_NO = A
    GOSUB Number_Play
    'GOSUB SOUND_PLAY_CHK


    IF  A = 16 THEN 	'О©╫ы╫О©╫ О©╫д©О©╫О©╫О©╫ф╟О©╫О©╫ О©╫О©╫О©╫О©╫О©╫ъ╦О©╫ О©╫О©╫О©╫О©╫
        GOSUB MOTOR_ON
        SPEED 10
        MOVE G6A,100, 140,  37, 145, 100, 100
        MOVE G6D,100, 140,  37, 145, 100, 100
        WAIT

        GOSUB О©╫Б╨╩О©╫з╪О©╫2
        GOSUB О©╫О©╫О©╫л╥О©╫ON
        GOSUB All_motor_mode3
        GOTO RX_EXIT
    ENDIF

    GOSUB GOSUB_RX_EXIT
    GOTO KEY16_1



    GOTO RX_EXIT
    '***************
KEY17: ' C
    ETX 4800, 17
    GOSUB О©╫с╦О©╫О©╫О©╫О©╫О©╫60О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY18: ' E
    ETX 4800, 18
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY19: 'P2
    ETX 4800, 19
    GOSUB  О©╫О©╫э©О©╫О©╫О©╫О©╫ъЁО©╫О©╫О©╫О©╫О©╫1cm
    GOTO RX_EXIT
    '***************
KEY20: 'B
    ETX 4800, 20
    GOSUB О©╫с╦О©╫О©╫О©╫О©╫О©╫О©╫О©╫30О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY21: ' О©╫О©╫
    ETX  4800,21
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫70О©╫О©╫

    GOTO RX_EXIT
    '***************
KEY22: ' *
    ETX 4800, 22
    '  GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫3
    'GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫20
    ' GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45
    GOTO О©╫О©╫э©ч╧ъ©О©╫О©╫О©╫О©╫О©╫1cm
    GOTO RX_EXIT
    '***************
KEY23: 'G
    ETX 4800, 23
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫й©О©╫О©╫О©╫О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY24: '#
    ETX 4800, 24
    GOSUB О©╫О©╫э©О©╫О©╫О©╫О©╫ъ©О©╫О©╫О©╫О©╫О©╫1cm
    GOTO RX_EXIT
    '***************
KEY25: 'P1
    ETX 4800, 25
    ' GOTO О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45
    ' GOTO О©╫О©╫О©╫О©╫О©╫О©╫3
    'GOTO О©╫О©╫О©╫О©╫О©╫О©╫20
    GOSUB О©╫О©╫э©ч╧ъЁО©╫О©╫О©╫О©╫О©╫1cm
    GOTO RX_EXIT
    '***************
KEY26: ' О©╫О©╫
    ETX  4800,26

<<<<<<< HEAD
    SPEED 5
<<<<<<< HEAD
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫
=======
=======
    'SPEED 5
<<<<<<< HEAD
<<<<<<< HEAD
<<<<<<< HEAD
<<<<<<< HEAD
>>>>>>> d2ab6ee (Chore: Л┐╓Л┌╜Л┐╓Л┌╜ К╙╗Л┘≤ Л┬≤Л═∙)
    GOSUB ╩Ч╩Х╩Ч╩Х
    'GOSUB ╧╟╟гаЩ╠Б
>>>>>>> 5f3e757 (Chore: Л┐╓Л┌╜Л┐╓Л┌╜ К╙╗Л┘≤ Л┬≤Л═∙)
=======
    'GOSUB ╩Ч╩Х╩Ч╩Х
    GOSUB ╧╟╟гаЩ╠Б
>>>>>>> 2bc48b6 (Chore: Л²▄Л└╠ Й╢─К═╗ К╙╗Л┘≤ Л┬≤Л═∙)
=======
    GOSUB ╩Ч╩Х╩Ч╩Х
    'GOSUB ╧╟╟гаЩ╠Б
>>>>>>> f330313 (Chore: stair_left_down К╛╢К╕▌, М▄■ К╡▄К╕╛К┼■ Й╟│К▐└ Л┬≤Л═∙)
=======
    'GOSUB ╩Ч╩Х╩Ч╩Х
    GOSUB ╧╟╟гаЩ╠Б
>>>>>>> 77312ab (Add: Л╢╛Л≤│.bas Л╤■Й╟─)
=======
    ╨╦гЮх╫╪Ж = 1
    GOSUB х╫╪Ж_╩Ч╩Х╩Ч╩Х
    'GOSUB ╧╟╟гаЩ╠Б
>>>>>>> cbc020c (Feat: М ÷Л┬≤_Л┐╓Л┌╜Л┐╓Л┌╜ Л╤■Й╟─)
    GOTO RX_EXIT
    '***************
KEY27: ' D
    ETX 4800, 27
    GOTO О©╫с╦О©╫О©╫О©╫О©╫О©╫О©╫О©╫60О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY28: ' О©╫О©╫
    ETX 4800, 28
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫45О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY29: ' О©╫О©╫
    ETX 4800, 29
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫110О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY30: ' О©╫О©╫
    ETX 4800, 30
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫30О©╫О©╫
    GOTO RX_EXIT
    '***************
KEY31: ' О©╫О©╫
    ETX 4800, 31
    GOSUB О©╫О©╫О©╫О©╫О©╫О©╫О©╫О©╫10О©╫О©╫
    GOTO RX_EXIT
    '***************

KEY32: ' F
    ETX 4800, 32
    GOSUB О©╫О©╫О©╫х╧О©╫О©╫О©╫О©╫О©╫
    GOTO RX_EXIT
    '***************
