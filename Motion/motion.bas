...
문열기걷기_2:
    ...
    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 보행횟수 THEN  GOTO 문열기걷기_2_stop
...
문열기걷기_5:
    ...
    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 보행횟수 THEN  GOTO 문열기걷기_5_stop
...
문열기걷기후진_3:
    ...
    ENDIF
    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 보행횟수 THEN  GOTO 문열기걷기후진_3_stop
    ERX 4800,A, 문열기걷기후진_4
    IF A <> A_old THEN
문열기걷기후진_3_stop:
        MOVE G6D,95,  85, 130, 100, 104
        MOVE G6A,104,  77, 146,  93,  102
...
문열기걷기후진_6:
    ...
    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 보행횟수 THEN  GOTO 문열기걷기후진_6_stop

    ERX 4800,A, 문열기걷기후진_1
    IF A <> A_old THEN  'GOTO 문열기걷기후진_멈춤
문열기걷기후진_6_stop:
        MOVE G6A,95,  85, 130, 100, 104
        MOVE G6D,104,  77, 146,  93,  102
        WAIT
...
문열고전진달리기50_3:
    ...
    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 보행횟수 THEN  GOTO 문열고전진달리기50_3_stop
...
문열고전진달리기50_6:
    ...
    ENDIF
    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 보행횟수 THEN  GOTO 문열고전진달리기50_6_stop
    ERX 4800,A, 문열고전진달리기50_1
    IF A <> A_old THEN
문열고전진달리기50_6_stop:
...
샤삭샤삭1: '왼발
    'HIGHSPEED SETON
    MOVE G6D,104,  77, 147, 93, 102
    MOVE G6A,95,  95, 140,  94,  104
    MOVE G6B, 100
    MOVE G6C, 100
    WAIT
샤삭샤삭2:

    MOVE G6A,99,    75, 146, 97,  100
    ...
    IF A <> A_old THEN
샤삭샤삭_2_stop:
        MOVE G6D,95,  87, 140, 97, 104
        MOVE G6A,104,  76, 145,  92,  102
        MOVE G6C, 100
        MOVE G6B,100
        WAIT
        HIGHSPEED SETOFF
...
샤삭샤삭4: '오른발
    MOVE G6A,104,  77, 147, 93, 102
    MOVE G6D,95,  95, 140,  94,  104
    MOVE G6C, 100
    MOVE G6B, 100
    WAIT

샤삭샤삭5:
    MOVE G6D,99,    75, 146, 97,  100
    MOVE G6A, 95,  77, 147,  93, 102
    WAIT
    ...
    IF A <> A_old THEN
샤삭샤삭5_stop:
        MOVE G6A,95,  87, 140, 97, 104
        MOVE G6D,104,  76, 145,  92,  102
        MOVE G6B, 100
        MOVE G6C,100
        WAIT
        HIGHSPEED SETOFF
